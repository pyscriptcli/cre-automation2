...
<head>
    ...
    <style>
        ...
        #scan-results-panel { position: absolute; top: 10px; right: 10px; z-index: 1000; background: #ffffff; width: 280px; max-height: calc(100vh - 20px); border-radius: 2px; border: 1px solid rgba(0, 51, 102, 0.1); background-clip: padding-box; display: flex; flex-direction: column; overflow: hidden; box-shadow: 0 4px 12px rgba(0, 51, 102, 0.08); }
        ...
        /* High-fidelity responsive 3D Drop-Pin Styling */
        .custom-pin-wrapper { transform: translate(-50%, -90%); position: relative; }
        
        /* Group Creator CSS Matrix Layout Panel */
        .group-generator-action-row { background: #f8fafc; border-bottom: 1px solid rgba(0,51,102,0.1); padding: 8px 12px; display: flex; flex-direction: column; gap: 6px; }
        .group-action-trigger-btn { background: #003366; color: #ffffff; border: none; font-family: 'Montserrat', sans-serif; font-size: 8.5px; font-weight: 800; text-transform: uppercase; padding: 6px; border-radius: 2px; cursor: pointer; letter-spacing: 0.5px; text-align: center; width: 100%; transition: background 0.15s; }
        .group-action-trigger-btn:hover { background: #C9AB4C; }
        
        .group-modal-window { background: #ffffff; border-top: 2px solid #C9AB4C; padding: 8px; display: none; flex-direction: column; gap: 6px; }
        .group-modal-window.active { display: flex !important; }
        .group-text-field-element { font-family: 'Montserrat', sans-serif; font-size: 9.5px; padding: 4px; border: 1px solid rgba(0,51,102,0.15); outline: none; color: #003366; width: 100%; box-sizing: border-box; }
        .group-checkbox-selection-matrix { max-height: 100px; overflow-y: auto; background: #f8fafc; border: 1px solid #eef2f6; padding: 4px; display: flex; flex-direction: column; gap: 4px; }
        .group-checkbox-label-item { font-size: 9px; font-weight: 600; display: flex; align-items: center; gap: 4px; color: #003366; cursor: pointer; }
        .group-checkbox-label-item input { accent-color: #003366; cursor: pointer; }
        
        .group-container-header-block { background: #f0f4f8; border-bottom: 1px solid rgba(0,51,102,0.08); padding: 6px 12px; display: flex; align-items: center; justify-content: space-between; font-family: 'Montserrat', sans-serif; font-size: 9.5px; font-weight: 800; color: #003366; text-transform: uppercase; letter-spacing: 0.5px; }
        .group-header-left-side { display: flex; align-items: center; gap: 6px; }
        .group-header-right-side { display: flex; align-items: center; gap: 4px; }
        .group-interactive-glyph { cursor: pointer; color: #888780; padding: 2px; font-weight: bold; }
        .group-interactive-glyph:hover { color: #AA2E20; }
    </style>
</head>
<body>
    <div id="map"></div>
    ...
    <div id="scan-results-panel">
        ...
        <div class="group-generator-action-row">
            <button class="group-action-trigger-btn" onclick="openLayerGroupCreationModal()">Create Group Layer</button>
            <div class="group-modal-window" id="lg-creator-modal-box">
                <input type="text" id="lg-group-name-input" class="group-text-field-element" placeholder="Enter Group Name...">
                <div class="group-checkbox-selection-matrix" id="lg-checkbox-layers-hook"></div>
                <div style="display: flex; gap: 4px;">
                    <button class="group-action-trigger-btn" style="background: #C9AB4C;" onclick="compileAndSaveCustomLayerGroup()">Save Group</button>
                    <button class="group-action-trigger-btn" style="background: #888780;" onclick="closeLayerGroupCreationModal()">Cancel</button>
                </div>
            </div>
        </div>
    ...
    <script>
        ...
        function saveProjectSettings(e) {
            ...
            const projectData = {
                ...
                radius: __RADIUS__,
                scanned_records: pts,
                custom_groups: customGroupsList
            };
        ...
        let customGroupsList = []; // Memory cache array tracking structural layer groups
        ...
        // Dynamic High-Fidelity 3D Needle Drop-Pin Icon generator equipped with linear SVG shadows
        const createPinIcon = (color) => {
            const svg = `
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 40 60" width="26" height="39" class="custom-pin-wrapper">
                <defs>
                    <filter id="svgShadow" x="-10%" y="-10%" width="130%" height="130%">
                        <feDropShadow dx="0" dy="3.5" stdDeviation="2.5" flood-color="#001F3F" flood-opacity="0.3"/>
                    </filter>
                </defs>
                <g filter="url(#svgShadow)">
                    <path d="M20 18 L20 48" stroke="${color}" stroke-width="3" stroke-linecap="round"/>
                    <path d="M20 18 L20 48" stroke="#FFFFFF" stroke-width="0.8" stroke-linecap="round"/>
                    <circle cx="20" cy="18" r="12" fill="${color}" stroke="#0A1520" stroke-width="1.2" />
                    <circle cx="20" cy="18" r="3.8" fill="#FFFFFF"/>
                </g>
            </svg>`;
            return L.divIcon({ html: svg, className: '', iconSize: [26, 39], iconAnchor: [13, 35], popupAnchor: [0, -35] });
        };
        ...
        Object.keys(categoryMap).forEach(key => {
            ...
            categoryMap[key].forEach(p => {
                ...
                if (p.name && p.name !== 'Unknown') {
                    marker.bindTooltip(p.name, { permanent: true, direction: 'top', offset: [0, -28], className: 'poi-text-label' });
                }
                p._marker = marker;
        ...
        // MULTI-SELECT GROUP LAYER LOGIC ARCHITECTURE IMPLEMENTATION
        window.openLayerGroupCreationModal = function() {
            const container = document.getElementById('lg-checkbox-layers-hook');
            const modal = document.getElementById('lg-creator-modal-box');
            container.innerHTML = '';
            
            const internalLayers = Object.keys(categoryMap).filter(k => {
                return !customGroupsList.some(g => g.layers.includes(k));
            });
            
            if(internalLayers.length === 0) {
                container.innerHTML = "<div style='font-size:8px; color:#888780; padding:4px;'>All layers grouped.</div>";
            } else {
                internalLayers.forEach(layer => {
                    const label = document.createElement('label');
                    label.className = 'group-checkbox-label-item';
                    label.innerHTML = `<input type="checkbox" value="${layer}"> ${layer}`;
        ...
        window.closeLayerGroupCreationModal = function() {
            document.getElementById('lg-creator-modal-box').classList.remove('active');
            document.getElementById('lg-group-name-input').value = '';
        };

        window.compileAndSaveCustomLayerGroup = function() {
            const nameInput = document.getElementById('lg-group-name-input').value.trim();
            const checkedBoxes = document.querySelectorAll('#lg-checkbox-layers-hook input:checked');
            
            if(!nameInput) { alert("Please supply a valid Group Name."); return; }
            if(checkedBoxes.length === 0) { alert("Select at least 1 layer to group."); return; }
            
            const selectedLayers = Array.from(checkedBoxes).map(cb => cb.value);
            
            customGroupsList.push({
                id: 'grp_' + Date.now(),
                name: nameInput,
            ...
            closeLayerGroupCreationModal();
            renderActiveWorkspaceCanvasDOM();
        };

        window.removeCustomLayerGroupWrapper = function(groupId) {
            customGroupsList = customGroupsList.filter(g => g.id !== groupId);
            renderActiveWorkspaceCanvasDOM();
        };

        window.toggleCustomGroupLayerVisibility = function(groupId, isVisible) {
            const group = customGroupsList.find(g => g.id === groupId);
            if(group) {
                group.visible = isVisible;
                group.layers.forEach(layerKey => {
                    const masterCheckbox = document.getElementById(`chk-layer-${layerKey}`);
                    if(masterCheckbox) masterCheckbox.checked = isVisible;
                    
                    if (isVisible) map.addLayer(layerGroupsRef[layerKey]);
                    else map.removeLayer(layerGroupsRef[layerKey]);
                });
            }
        };
        ...
        // RENDER WORKSPACE LAYER CONTROLS DOM WITH DYNAMIC GROUPING
        function renderActiveWorkspaceCanvasDOM() {
            const listBox = document.getElementById('results-list-box');
            document.getElementById('results-count').innerText = pts.length;
            
            if (pts.length === 0) {
                listBox.innerHTML = "<div style='font-size:9px; padding:12px; color:#888780;'>No elements captured.</div>";
                return;
            }
            ...
            // 1. First Render Grouped Layers Block
            customGroupsList.forEach(group => {
                htmlPayload += `
                <div style="border: 1px solid rgba(0,51,102,0.15); margin-bottom: 6px; border-radius: 2px; overflow:hidden;">
                    <div class="group-container-header-block">
                        <div class="group-header-left-side">
                            <input type="checkbox" ${group.visible ? 'checked' : ''} onclick="toggleCustomGroupLayerVisibility('${group.id}', this.checked)">
                            <span>📦 ${group.name}</span>
                        </div>
                        <div class="group-header-right-side">
                            <span class="group-interactive-glyph" title="Ungroup Layer Cluster" onclick="removeCustomLayerGroupWrapper('${group.id}')">✕</span>
                        </div>
                    </div>
                `;
                
                group.layers.forEach(layerKey => {
                    if(!categoryMap[layerKey]) return;
                    const dotColor = categoryColors[layerKey];
                    htmlPayload += buildLayerCategoryMarkup(layerKey, dotColor, trashSvg, true, group.visible);
                });
                
                htmlPayload += `</div>`;
            });

            // 2. Render Independent / Ungrouped Categories Base
            Object.keys(categoryMap).forEach(catName => {
                const isGrouped = customGroupsList.some(g => g.layers.includes(catName));
                if(isGrouped) return;
                
                const dotColor = categoryColors[catName];
                htmlPayload += buildLayerCategoryMarkup(catName, dotColor, trashSvg, false, true);
            });

            listBox.innerHTML = htmlPayload;
        }

        function buildLayerCategoryMarkup(catName, dotColor, trashSvg, isSubLayer, parentVisible) {
            return `
                <div class="layer-category-block" id="cat-block-${catName}" style="${isSubLayer ? 'padding-left: 8px; background: #fafbfc;' : ''}">
                    <div class="layer-category-header" onclick="toggleAccordionCollapse('${catName}')">
                        <div class="layer-header-left">
                            <input type="checkbox" id="chk-layer-${catName}" ${parentVisible ? 'checked' : ''} onclick="event.stopPropagation(); toggleCategoryVisibility('${catName}', this.checked)">
