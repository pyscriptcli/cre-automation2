    ...
    for endpoint in endpoints:
        for retries in range(5, 0, -1):
            try:
                ...
                data = res.json()
                if not data or 'elements' not in data: raise ValueError("Malformed JSON")
                
                return [{
                    'lat': float(el.get('lat') or el.get('center', {}).get('lat')),
                    'lon': float(el.get('lon') or el.get('center', {}).get('lon')),
                    'name': str(el.get('tags', {}).get('name', 'Unknown')),
                    'type': str(el.get('tags', {}).get('amenity') or el.get('tags', {}).get('shop') or el.get('tags', {}).get('building') or 'Node'),
                    'tags': el.get('tags', {})
                } for el in data['elements'] if (el.get('lat') or el.get('center', {}).get('lat')) and (el.get('lon') or el.get('center', {}).get('lon'))]
                
            except Exception as e:
                if retries == 1: break
                time.sleep(2 ** (5 - retries) + random.uniform(0, 0.5))

    logging.info("Falling back to OSMnx...")
    try:
        import osmnx as ox
        tags_dict = {tag.replace('"', '').split('=', 1)[0]: [x.strip() for x in tag.replace('"', '').split('=', 1)[1].split('|')] if '=' in tag and '|' in tag else (tag.replace('"', '').split('=', 1)[1] if '=' in tag else True) for tag in tags}
        gdf = ox.geometries_from_point((lat, lon), tags_dict, dist=radius)
        return [{
            'lat': float(row.geometry.y if row.geometry.geom_type == 'Point' else row.geometry.centroid.y),
            'lon': float(row.geometry.x if row.geometry.geom_type == 'Point' else row.geometry.centroid.x),
...
st.markdown(
    ...
    * { font-family: 'Century Gothic Custom', -apple-system, BlinkMacSystemFont, "SF Pro Display", sans-serif !important; }
    ...
    header, #MainMenu, footer, [data-testid="stHeader"] { display: none !important; height: 0 !important; margin: 0 !important; padding: 0 !important; }
...
def fetch_projects():
    try:
        ...
        res = requests.get(url, headers={"apikey": SUPABASE_KEY, "Authorization": f"Bearer {SUPABASE_KEY}", "Content-Type": "application/json", "Prefer": "return=representation"}, timeout=6)
...
POI_CONFIG = {
    ...
    "RETAIL": [['Mall/Department Store', '"shop"~"mall|department_store",i'], ['Supermarket', '"shop"~"market|grocery",i'], ['Convenience Store', '"shop"="convenience"'], ['Pharmacy', '"amenity"="pharmacy"'], ['Hardware', '"shop"~"hardware|doityourself",i'], ['General Shops', '"shop"~"boutique|clothes|shoes",i'], ['Marketplace', '"amenity"="marketplace"']],
    "FOOD, BEVERAGE & HOSPITALITY": [['Restaurant', '"amenity"="restaurant"'], ['Cafe/Coffee Shop', '"amenity"~"cafe|coffee",i'], ['Fast Food', '"amenity"="fast_food"'], ['Bar/Pub/Nightclub', '"amenity"~"bar|pub|nightclub",i'], ['Bakery/Pastry', '"shop"="bakery"'], ['Food court', '"amenity"="food_court"'], ['Hotel', '"tourism"="hotel"'], ['Hostel', '"tourism"="hostel"']],
    ...
    "INDUSTRIAL & LOGISTICS": [['Expressway Exits', '"highway"~"motorway_junction|toll_gantry",i'], ['Ports & Terminals', '"industrial"="port"'], ['Manufacturing Plants', '"industrial"~"factory|manufacturing|processing",i'], ['Warehouses & Depots', '"building"~"warehouse|depot",i'], ['Industrial Parks', '"landuse"~"industrial|industrial_estate",i']],
    ...
    "GOVERNMENT, EDUCATION & INFRASTRUCTURE": [['City Hall', '"amenity"="townhall"'], ['Airport Terminal', '"aeroway"~"terminal|aerodrome",i'], ['University/College', '"amenity"~"university|college",i'], ['K-12 School', '"amenity"="school"'], ['Post Office', '"amenity"="post_office"']],
    "LEISURE, SPORTS & PUBLIC SPACES": [['Church', '"religion"="christian"'], ['Mosque', '"religion"="muslim"'], ['Cinema', '"amenity"="cinema"'], ['Fuel', '"amenity"="fuel"'], ['Parking', '"amenity"="parking"'], ['Sports centre', '"leisure"="sports_centre"'], ['Busstop', '"highway"="bus_stop"']]
}

THEMES = {
    "Midnight Blue": {"overlay": "#0a1628", "text": "#d9b451", "land": "#0d1830", "landcover": "#0f1d33", "water": "#0a1424", "waterway": "#081120", "parks": "#142440", "buildings": "#8e7258", "aeroway": "#152640", "rail": "#d9b451", "rd_express": "#ffaa00", "rd_major": "#e8b84a", "rd_secondary": "#c99c37", "rd_tertiary": "#7d5f14", "rd_min_md": "#46463e", "rd_min_lo": "#2f2f2a", "rd_path": "#4a4333", "rd_case": "#685c37", "sec_opacity": 0.8, "ter_opacity": 0.65, "building_opacity": 0.35, "boundary": "#ff1e1e", "muted": "#8b949e"},
    "Monochrome": {"overlay": "#ece9e2", "text": "#2d2a26", "land": "#ece9e2", "landcover": "#e5e2da", "water": "#cdd7db", "waterway": "#bac6cb", "parks": "#e2dfd7", "buildings": "#dedad2", "aeroway": "#dbd7cf", "rail": "#1a1816", "rd_express": "#1a1816", "rd_major": "#2e2a25", "rd_secondary": "#47423b", "rd_tertiary": "#716b61", "rd_min_md": "#8a8377", "rd_min_lo": "#9e978d", "rd_path": "#b0a99f", "rd_case": "#1a1816", "sec_opacity": 0.85, "ter_opacity": 0.7, "building_opacity": 0.6, "boundary": "#ff1e1e", "muted": "#716b61"},
    "White Gold": {"overlay": "#ffffff", "text": "#a07d1c", "land": "#fafafa", "landcover": "#f1f1ec", "water": "#d4dadc", "waterway": "#c2c9cc", "parks": "#e6ebe4", "buildings": "#d8d8d4", "aeroway": "#e4e4e4", "rail": "#c99c37", "rd_express": "#f59e0b", "rd_major": "#e5a91d", "rd_secondary": "#b08a24", "rd_tertiary": "#9c7a1a", "rd_min_md": "#e0be74", "rd_min_lo": "#ead9b0", "rd_path": "#e6dabd", "rd_case": "#b08a24", "sec_opacity": 0.7, "ter_opacity": 0.6, "building_opacity": 0.5, "boundary": "#ff1e1e", "muted": "#6b7280"},
}

def w(*stops):
    stops = stops[0] if len(stops) == 1 and isinstance(stops[0], (list, tuple)) and len(stops[0]) > 0 and isinstance(stops[0][0], (list, tuple)) else stops
    return ["interpolate", ["exponential", 1.2], ["zoom"]] + [item for stop in stops for item in (stop[0], stop[1])]

def road_layer(p, lid, classes, color, widths, minzoom=0, casing=False, opacity=1.0):
    lyr = {"id": f"{lid}_casing" if casing else lid, "type": "line", "source": "omt", "source-layer": "transportation", "filter": ["match", ["get", "class"], classes, True, False], "layout": {"line-cap": "round", "line-join": "round"}, "paint": {"line-color": p["rd_case"] if casing else color, "line-width": w([(z, val + (1.8 if casing else 0)) for z, val in widths]), "line-opacity": opacity}}
    if minzoom: lyr["minzoom"] = minzoom
    return lyr

def vector_style(p):
    return {
        "version": 8, "glyphs": "https://tiles.openfreemap.org/fonts/{fontstack}/{range}.pbf", "sources": {"omt": {"type": "vector", "url": "https://tiles.openfreemap.org/planet"}},
        "layers": [
...
ALL_STYLES = {
    "Midnight Blue": vector_style(THEMES["Midnight Blue"]), "Monochrome": vector_style(THEMES["Monochrome"]), "White Gold": vector_style(THEMES["White Gold"]),
    ...
    "OSM": raster_style(["https://tile.openstreetmap.org/{z}/{x}/{y}.png"], "#f2efe9", 19), "Satellite": raster_style(["https://server.arcgisonline.com/ArcGIS/rest/services/World_Imagery/MapServer/tile/{z}/{y}/{x}"], "#000000", 19)
}
...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; user-select: none; font-family: 'Century Gothic Custom', -apple-system, BlinkMacSystemFont, "SF Pro Display", sans-serif; }
        html, body { margin: 0; padding: 0; width: 100vw; height: 100vh; overflow: hidden; background: var(--bg-main); }
        ...
        #top-toolbar-bar {
            ...
            border: 1px solid var(--border-color); border-radius: 36px; padding: 4px 10px;
        ...
        .tb-btn.primary-active { background: var(--primary); color: #ffffff; }
        ...
        #project-name-display { font-weight: 700; color: var(--accent); font-size: 12px; max-width: 140px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; cursor: pointer; }
        ...
        .save-badge.saved { color: var(--success); border-color: rgba(63, 185, 80, 0.4); }
        ...
        .dimension-mode-btn.active { background: var(--primary); color: #ffffff; }
        
        .bound-select-row { display: flex; gap: 6px; margin-top: 4px; position: relative; }
        .bound-select-row input[type=text] { flex: 1; background: rgba(0, 0, 0, 0.4); border: 1px solid var(--border-color); color: var(--text-main); padding: 6px 8px; border-radius: 8px; font-size: 11px; }
        
        .autocomplete-list { position: absolute; top: 100%; left: 0; right: 0; z-index: 1001; background: #0f172a; border: 1px solid var(--border-color); border-radius: 8px; max-height: 200px; overflow-y: auto; display: none; margin-top: 4px; box-shadow: 0 8px 16px rgba(0,0,0,0.5); }
        ...
        .badge-count { background: var(--primary); color: #ffffff; border-radius: 12px; font-size: 11px; padding: 1px 8px; font-weight: 600; }
        ...
        .group-container.drop-hover { border-color: var(--accent); background: rgba(56, 189, 248, 0.12); }
        ...
        .layer-card.selected { border-color: var(--accent); background: rgba(56, 189, 248, 0.1); }
        ...
        .layer-name-input:focus { border-color: var(--primary); background: rgba(0,0,0,0.4); outline: none; }
        ...
        #ungrouped-zone.drop-hover { border-color: var(--accent); background: rgba(56, 189, 248, 0.08); }
        ...
        .trade-controls select { background: #0f172a; color: var(--text-main); border: 1px solid var(--border-color); border-radius: 8px; padding: 6px; font-size: 11px; }
        .trade-btn { background: var(--primary); color: #ffffff; border: none; border-radius: 8px; padding: 7px; font-weight: 600; cursor: pointer; font-size: 11px; transition:0.2s;}
        .trade-btn:hover { background: var(--primary-hover); }
        ...
        .float-card {
            ...
            background-color: var(--bg-panel);
            border: 1px solid var(--border-color); border-radius: 18px; padding: 14px;
        ...
        .float-card input[type=range] { accent-color: var(--primary); width: 110px; cursor: pointer; }
        ...
        .float-card input[type=text], .float-card select { background: rgba(0,0,0,0.4); color: var(--text-main); border: 1px solid var(--border-color); border-radius: 8px; padding: 6px 8px; font-size: 12px; transition:0.2s; outline:none; }
        .float-card input[type=text]:focus { border-color: var(--accent); }
        ...
        .icon-grid button { width: 36px; height: 36px; display: grid; place-items: center; border: 1px solid rgba(255,255,255,0.1); border-radius: 8px; background: rgba(255,255,255,0.05); color: var(--text-muted); cursor: pointer; transition:0.2s;}
        .icon-grid button.active { border-color: var(--primary); background: var(--primary); color: #ffffff; }
        
        .maplibregl-popup-content { background: var(--bg-panel) !important; color: var(--text-main) !important; border: 1px solid rgba(255, 255, 255, 0.15) !important; border-radius: 12px !important; padding: 10px !important; box-shadow: 0 12px 32px rgba(0,0,0,0.7) !important; font-size: 11px !important; max-width: 280px !important; }
        ...
        .tag-table th { background: rgba(255,255,255,0.06); color: var(--accent); }
        .tag-table td { word-break: break-all; }
        
        #hint-toast { position: fixed; bottom: 24px; left: 50%; transform: translateX(-50%); z-index: 1001; background-color: var(--bg-panel); color: var(--text-main); border: 1px solid var(--border-color); border-radius: 20px; padding: 7px 18px; font-size: 12px; box-shadow: 0 8px 24px rgba(0,0,0,0.5); display: none; font-weight:600; }
        ...
        .ctx-item { display: flex; align-items: center; gap: 8px; padding: 8px 10px; font-size: 12px; color: var(--text-main); cursor: pointer; border-radius: 6px; }
        ...
        #launcher-modal-scrim { position: fixed; inset: 0; z-index: 9999; display: flex; align-items: center; justify-content: center; background-color: var(--bg-panel); opacity: 0; pointer-events: none; transition: opacity 0.2s ease; }
        #launcher-modal-scrim.visible { opacity: 1; pointer-events: auto; }
        
        .ios26-card { width: 90%; max-width: 440px; max-height: 82vh; background-color: var(--bg-panel); border: 1px solid rgba(255, 255, 255, 0.16); border-radius: 24px; box-shadow: 0 32px 80px -12px rgba(0, 0, 0, 0.85); display: flex; flex-direction: column; overflow: hidden; color: #ffffff; }
        ...
        .ios26-input { background: rgba(0, 0, 0, 0.35); border: 1px solid var(--border-color); border-radius: 14px; padding: 10px 14px; color: #ffffff; font-size: 13px; outline: none; }
        .ios26-input:focus { border-color: var(--accent); }
        ...
        .ios26-action-btn { background: var(--primary); color: #ffffff; border: none; border-radius: 14px; padding: 11px; font-weight: 700; font-size: 13px; cursor: pointer; box-shadow: 0 8px 24px rgba(49, 109, 202, 0.4); }
        .ios26-action-btn:hover { background: var(--primary-hover); }
        
        .file-input-label { display: inline-block; background: var(--primary); color: #fff; padding: 6px 12px; border-radius: 6px; cursor: pointer; font-size: 11px; font-weight: 600; transition:0.2s; }
        ...
        .custom-query-collapse-header { display: flex; align-items: center; justify-content: space-between; cursor: pointer; font-weight: 600; color: var(--text-main); font-size: 12px; margin-top: 8px;}
        ...
        .attr-table th { position: sticky; top: 0; background: #0f172a; color: var(--text-main); padding: 8px; text-align: left; border-bottom: 1px solid rgba(255,255,255,0.1); z-index: 10; font-size: 11px; }
        ...
        .attr-table input[type="text"]:focus { border-color: var(--accent); outline: none; }
        ...
        .attr-img-placeholder { width: 80px; height: 80px; border-radius: 6px; border: 1px dashed rgba(255,255,255,0.3); display: flex; align-items: center; justify-content: center; font-size: 10px; color: var(--text-muted); cursor: pointer; text-align: center; }
        ...
        .btn-eyedropper { width: 24px; height: 24px; display: grid; place-items: center; background: rgba(255,255,255,0.08); border: 1px solid rgba(255,255,255,0.15); border-radius: 4px; color: var(--text-muted); cursor: pointer; padding: 0; }
...
<body>
        ...
        <div class="panel-content">
            ...
            <div class="acc-item">
                ...
                <div class="acc-body hidden" id="body-boundaries">
                    ...
                    <div style="font-weight:600; font-size:11px; color:var(--text-main); margin-top:4px;">Highlight Administrative Boundary</div>
        ...
        <div class="panel-content">
            <div class="layers-heading">
                ...
                <div style="display:flex; align-items:center; gap:4px;">
                    ...
                    <button class="icon-action-btn" id="btnDeleteSelectedGlobal" title="Delete Selected Layers" style="color:var(--danger);">
    ...
    <div id="popup-route-settings" class="float-card right-card">
        <div style="display:flex; justify-content:space-between; align-items:center;">
            <span style="font-weight:700; color:var(--text-main);">Route (OSRM)</span>
        ...
        <div class="f-row"> <span>Auto-Finish Toggle</span>
            <label style="display:flex; align-items:center; gap:4px; font-size:11px; cursor:pointer;">
                <input type="checkbox" id="rAutoFinish" checked style="accent-color:var(--primary);"> On double-click
            </label>
        </div>
    ...
    <div id="popup-shape-editor" class="float-card right-card">
        ...
        <div id="routeEditorControls" style="display:none; border-top:1px solid rgba(255,255,255,0.1); margin-top:8px; padding-top:8px;">
            ...
            <div class="f-row" style="margin-top:4px;"> <span>Stats</span> <span id="eRouteStats" style="font-size:11px; font-weight:700; color:var(--accent);">-</span></div>
        ...
        <div style="display:flex; justify-content:space-between; margin-top:8px;">
            ...
            <button id="eDoneBtn" style="background:var(--primary); color:#fff; border:none; padding:6px 16px; border-radius:6px; cursor:pointer;">Done</button>
        </div>
    </div>

    <div id="popup-custom-map" class="float-card right-card">
        <div style="display:flex; justify-content:space-between; align-items:center;">
            <span style="font-weight:700; color:var(--text-main);">Vector & Basemap Style</span>
    ...
    <div id="trade-area-modal" class="modal-scrim" style="position: fixed; inset: 0; z-index: 2000; display: none; align-items: center; justify-content: center; background-color: rgba(0,0,0,0.6);">
        <div class="float-card open" style="width:500px; max-width:90vw; max-height:85vh; padding:16px;">
            ...
            <div style="margin-top:8px;">
                ...
                <input type="text" id="customPoiSearchInput" placeholder="e.g. amenity=dentist or ev_charging" style="width:100%; background:rgba(0,0,0,0.4); color:var(--text-main); border:1px solid var(--border-color); border-radius:8px; padding:6px; font-size:11px;">
            </div>
            ...
            <div id="customQueryBody" style="display:none;">
                <textarea id="overpassQueryInput" rows="4" style="background:rgba(0,0,0,0.4); color:var(--text-main); border:1px solid var(--border-color); border-radius:8px; padding:8px; font-size:12px; width:100%;"></textarea>
    ...
    <div id="popup-feature-info" class="float-card">
        <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:6px;">
            <span style="font-weight:700; color:var(--text-main); font-size:14px;" id="featureInfoName">Feature Name</span>
        ...
        <div class="attr-table-container" style="max-height:40vh;">
            <table class="tag-table" id="featureInfoTable">
            </table>
        </div>
        <div style="display:flex; justify-content:flex-end; gap:6px; margin-top:8px; border-top:1px solid rgba(255,255,255,0.08); padding-top:6px;">
             <button id="btnTableFromInfo" style="background:rgba(255,255,255,0.1); color:var(--text-muted); border:1px solid rgba(255,255,255,0.1); border-radius:4px; padding:3px 8px; font-size:10px; cursor:pointer;">Open Table</button>
        </div>
    </div>
    ...
    <div id="popup-attribute-table" class="float-card">
        <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:6px;">
            <span style="font-weight:700; color:var(--text-main);" id="attrTableTitle">Attributes</span>
            <button class="card-btn" id="closeAttrTableBtn">✕</button>
        </div>
        <div style="display:flex; gap:8px; margin-bottom:8px; align-items:center;">
            <input type="text" id="attrTableSearch" placeholder="Find in table (filter)..." style="flex:1; background:rgba(0,0,0,0.4); color:var(--text-main); border:1px solid var(--border-color); border-radius:8px; padding:6px 10px; font-size:11px;">
    ...
    <div id="map-context-menu">
        ...
        <div class="ctx-item" id="ctx-delete" style="display:none; color:var(--danger);">
    ...
    try {
        ...
        const undo = () => {
            ...
            syncDraw(); renderMyLayers(); setSaveBadgeStatus('unsaved'); hint('Undo');
        };
        ...
        const redo = () => {
            ...
            fid = features.reduce((max, f) => Math.max(max, f.id || 0), 0);
            syncDraw(); renderMyLayers(); setSaveBadgeStatus('unsaved'); hint('Redo');
        };

        const closeFloatingCards = () => {
            ['popup-marker-settings','popup-text-settings','popup-shape-editor','popup-custom-map','popup-search','popup-route-settings','browser-panel','mylayers-panel','popup-feature-info','popup-attribute-table'].forEach(id => {
                const el = $(id);
        ...
        function getNextUntitledProjectName() {
            ...
            ALL_PROJECTS.forEach(p => {
                ...
                if (match) maxN = maxN > int(match[1]) ? maxN : parseInt(match[1], 10);
            });
            ...
            container.innerHTML = ALL_PROJECTS.map(p => `
                <div class="ios26-proj-item">
                    ...
                    <div style="display:flex; align-items:center; gap:6px;">
                        ...
                        <button class="card-btn" onclick="deleteProjectFromLauncher(event, '${p.id}', '${(p.name || '').replace(/'/g, "\\'")}')" title="Delete" style="color:var(--danger);"><svg viewBox="0 0 24 24" width="13" height="13" fill="none" stroke="currentColor" stroke-width="2"><polyline points="3 6 5 6 21 6"></polyline><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"></path></svg></button>
                    </div>
                </div>
            `).join('');
        }
        ...
        window.loadProjectDirectly = function(projectId) {
            ...
            features.forEach(f => { if (f.kind === 'marker') f.props.iconKey = f.props.iconKey || getIconKey(f.props.shape || 'pin', f.props.color || '#1e40af'); });
        ...
        $('btn-create-project-submit').onclick = async () => {
            ...
            try {
                ...
                if (res.ok) {
                    ...
                    currentProjectId = "local-temp"; currentProjectName = pName; $('project-name-display').textContent = pName; features = []; customGroups = {"Trade Area Scan": {collapsed: false, ids: []}}; map.setCenter(centerLL); closeHomeDialog();
                }
        ...
        const ICON_SVGS = { pin: '<path d="M12 21s-7-6-7-11a7 7 0 0 1 14 0c0 5-7 11-7 11z"></path><circle cx="12" cy="10" r="2.5"></circle>', star: '<path d="M12 2l2.9 6.3 6.9.8-5.1 4.7 1.4 6.8-6.1-3.4-6.1 3.4 1.4-6.8L2.2 9.1l6.9-.8z"></path>', circle: '<circle cx="12" cy="12" r="8"></circle>', square: '<rect x="5" y="5" width="14" height="14"></rect>', flag: '<path d="M6 21V4"></path><path d="M6 4l12 3-12 3"></path>', heart: '<path d="M12 20s-7-4.6-7-10a4 4 0 0 1 7-2.5A4 4 0 0 1 19 10c0 5.4-7 10-7 10z"></path>', pinball: '<circle cx="12" cy="10" r="7"></circle><line x1="12" y1="17" x2="12" y2="22"></line>' };
        ...
        $('markerIconGrid').querySelectorAll('button').forEach(b => b.onclick = () => { markerShape = b.dataset.s; $('markerIconGrid').querySelectorAll('button').forEach(x => x.classList.toggle('active', x === b)); customMarkerImageKey = null; markDirty(); });
        ...
        const syncDraw = () => { if (map.getSource('draw')) map.getSource('draw').setData(fc(features)); syncVertexHandles(); syncLabels(); };
        ...
        function syncLabels() {
            ...
            features.forEach(f => {
                ...
                if (f.kind === 'route' && f.props.metadata) {
                    ...
                    labelText = `${dist > 1000 ? (dist/1000).toFixed(2)+' km' : Math.round(dist)+' m'} · ${dur > 3600 ? (dur/3600).toFixed(1)+' hr' : Math.round(dur/60)+' min'}`;
                ...
                if (f.geometry.type === 'Point') {
                    ...
                    coords = pos === 'top' ? [x, y + d] : pos === 'bottom' ? [x, y - d] : pos === 'left' ? [x - d, y] : pos === 'right' ? [x + d, y] : [x, y];
                } else {
                    const b = calcBounds(f);
                    if (!b) return;
                    const cx = (b[0][0] + b[1][0]) / 2, cy = (b[0][1] + b[1][1]) / 2;
                    coords = pos === 'top' ? [cx, b[1][1]] : pos === 'bottom' ? [cx, b[0][1]] : pos === 'left' ? [b[0][0], cy] : pos === 'right' ? [b[1][0], cy] : [cx, cy];
                }
        ...
        function pulseFeature(f) {
            ...
            function frame(t) {
                const prog = ((t - start) % 800) / 800;
                if (map.getLayer('pulse-circle')) { map.setPaintProperty('pulse-circle', 'circle-radius', 8 + prog * 55); map.setPaintProperty('pulse-circle', 'circle-opacity', 0.9 * (1 - prog)); }
                (t - start) < 1600 ? requestAnimationFrame(frame) : src.setData({ type: 'FeatureCollection', features: [] });
            }
        ...
        function syncVertexHandles() {
            ...
            if (f && f.props.visible !== 0) {
                if (f.kind === 'circle' && f.props.centerCoord && f.props.radiusMeters) {
                    handleFeats.push({ type: 'Feature', geometry: { type: 'Point', coordinates: [f.props.centerCoord[0] + (f.props.radiusMeters / (111320 * Math.cos(f.props.centerCoord[1]*Math.PI/180))), f.props.centerCoord[1]] }, properties: { polyId: f.id, isRadiusHandle: true } });
                } else if (['polygon','rectangle'].includes(f.kind) && f.geometry && f.geometry.coordinates && f.geometry.coordinates[0]) {
                    for (let i = 0; i < f.geometry.coordinates[0].length - 1; i++) handleFeats.push({ type: 'Feature', geometry: { type: 'Point', coordinates: f.geometry.coordinates[0][i] }, properties: { polyId: f.id, vIdx: i } });
                ...
                const b = calcBounds(f);
                if (b) handleFeats.push({ type: 'Feature', geometry: { type: 'Point', coordinates: [(b[0][0] + b[1][0]) / 2, b[1][1] + ((b[1][1] - b[0][1]) * 0.25 || 0.001)] }, properties: { polyId: f.id, isRotHandle: true } });
            }
        ...
        function renderDraft() {
            ...
            draft.forEach((p, i) => f.push(pt(p, i === 0 && activeTool === 'polygon', i === draft.length - 1 && activeTool === 'route' && draft.length > 0)));
            ...
            if (activeTool === 'circle' && draft.length === 1 && cursorLL) {
                ...
                f.push(ln(coords[0]));
                hint(`Radius: ${r > 1000 ? (r/1000).toFixed(2)+' km' : Math.round(r)+' m'} · Click to finalize`);
            }
        ...
        map.on('load', () => {
            ...
            setupColorPicker('eBorderColorCtrl', '#38bdf8', col => { const f = features.find(x => x.id === selectedId); if (f) { f.props.borderColor = col; f.props.color = col; if (f.kind === 'marker' && !customMarkerImageKey) f.props.iconKey = getIconKey(f.props.shape || 'pin', col); syncDraw(); markDirty(); } });
            ...
            const myLayersList = $('my-layers-list');
            myLayersList.addEventListener('click', e => {
                const btn = e.target.closest('.card-btn, button[data-act]');
                if (btn) {
                    const act = btn.dataset.act, g = btn.dataset.group, id = btn.dataset.id ? parseInt(btn.dataset.id, 10) : null;
                    if (act === 'groupToggleCollapse') { customGroups[g].collapsed = !customGroups[g].collapsed; renderMyLayers(); return; }
                    if (act === 'groupStyle') { const p = myLayersList.querySelector(`.group-styling-panel[data-group="${g}"]`); if(p) p.classList.toggle('open'); return; }
                    if (act === 'groupEye') { const anyVis = features.some(f => customGroups[g].ids.includes(f.id) && f.props.visible); features.forEach(f => { if (customGroups[g].ids.includes(f.id)) f.props.visible = anyVis ? 0 : 1; }); syncDraw(); renderMyLayers(); markDirty(); return; }
                    if (act === 'groupDel') { delete customGroups[g]; renderMyLayers(); markDirty(); return; }
                    if (act === 'groupSelectAll') { const allSelected = customGroups[g].ids.every(i => selectedLayerIds.has(i)); customGroups[g].ids.forEach(i => allSelected ? selectedLayerIds.delete(i) : selectedLayerIds.add(i)); renderMyLayers(); return; }
                    if (!id) return;
                    const f = features.find(x => x.id === id);
                    if (!f) return;
                    if (act === 'table') { openAttributeTable(id); return; }
                    if (act === 'eye') { f.props.visible = f.props.visible ? 0 : 1; syncDraw(); renderMyLayers(); markDirty(); return; }
                    if (act === 'del') { features = features.filter(x => x.id !== id); for (const grp in customGroups) customGroups[grp].ids = customGroups[grp].ids.filter(xId => xId !== id); selectedLayerIds.delete(id); if (selectedId === id) selectedId = null; syncDraw(); renderMyLayers(); markDirty(); return; }
                    if (act === 'zoom') { const bnd = calcBounds(f); if (bnd) { map.fitBounds(bnd, { padding: 60, maxZoom: 18 }); pulseFeature(f); } }
                }
                if (e.target.id === 'btnSelectAllUngrouped' || e.target.closest('#btnSelectAllUngrouped')) {
                    const groupedIds = new Set();
                    for (const g in customGroups) customGroups[g].ids.forEach(i => groupedIds.add(i));
                    const looseFeats = features.filter(f => !groupedIds.has(f.id));
                    const allSelected = looseFeats.every(f => selectedLayerIds.has(f.id));
                    looseFeats.forEach(f => allSelected ? selectedLayerIds.delete(f.id) : selectedLayerIds.add(f.id));
                    renderMyLayers();
                }
                if (e.target.classList.contains('grp-style-apply')) {
                    const panel = e.target.closest('.group-styling-panel');
                    const gName = panel.dataset.group, color = panel.querySelector('.grp-style-color').value, shape = panel.querySelector('.grp-style-shape').value, size = parseFloat(panel.querySelector('.grp-style-size').value);
                    if (customGroups[gName]) {
                        customGroups[gName].ids.forEach(id => { const f = features.find(x => x.id === id); if (f && f.kind === 'marker') { f.props.color = color; f.props.borderColor = color; if (shape) f.props.shape = shape; f.props.iconSize = size; f.props.iconKey = getIconKey(f.props.shape || 'pin', color); } });
                        syncDraw(); renderMyLayers(); markDirty(); hint(`Styling applied to "${gName}"`);
                    }
                }
            });

            myLayersList.addEventListener('change', e => {
                if (e.target.classList.contains('layer-select-check')) {
                    const id = parseInt(e.target.dataset.id, 10);
                    e.target.checked ? selectedLayerIds.add(id) : selectedLayerIds.delete(id);
                    const card = e.target.closest('.layer-card');
                    if (card) e.target.checked ? card.classList.add('selected') : card.classList.remove('selected');
                }
                if (e.target.dataset.act === 'labelToggle') { const f = features.find(x => x.id === parseInt(e.target.dataset.id, 10)); if (f) { f.props.showLabel = e.target.checked; syncDraw(); markDirty(); } }
                if (e.target.dataset.act === 'labelPos') { const f = features.find(x => x.id === parseInt(e.target.dataset.id, 10)); if (f) { f.props.labelPos = e.target.value; syncDraw(); markDirty(); } }
                if (e.target.classList.contains('group-title-input')) {
                    const oldN = e.target.dataset.oldname, newN = e.target.value.trim();
                    if (newN && newN !== oldN && !customGroups[newN]) { customGroups[newN] = customGroups[oldN]; delete customGroups[oldN]; renderMyLayers(); markDirty(); } else { e.target.value = oldN; }
                }
                if (e.target.classList.contains('layer-name-input')) {
                    const f = features.find(x => x.id === parseInt(e.target.dataset.id, 10));
            ...
            let draggedIdsGlobal = [];
            myLayersList.addEventListener('dragstart', e => {
                const card = e.target.closest('.layer-card');
                if (card) {
                    const cardId = parseInt(card.dataset.id, 10);
                    draggedIdsGlobal = selectedLayerIds.has(cardId) && selectedLayerIds.size > 1 ? Array.from(selectedLayerIds) : [cardId];
                    e.dataTransfer.setData('text/plain', JSON.stringify(draggedIdsGlobal)); e.dataTransfer.effectAllowed = 'move';
                }
            });
            myLayersList.addEventListener('dragover', e => {
                e.preventDefault(); e.dataTransfer.dropEffect = 'move';
                const dropZone = e.target.closest('.layer-card, .group-container, #ungrouped-zone');
                if (dropZone) dropZone.classList.add('drop-hover');
            });
            myLayersList.addEventListener('dragleave', e => {
                const dropZone = e.target.closest('.layer-card, .group-container, #ungrouped-zone');
                if (dropZone) dropZone.classList.remove('drop-hover');
            });
            myLayersList.addEventListener('drop', e => {
                e.preventDefault(); e.stopPropagation();
                const dropZone = e.target.closest('.layer-card, .group-container, #ungrouped-zone');
                if (dropZone) dropZone.classList.remove('drop-hover');
                if (dropZone && dropZone.classList.contains('group-container')) {
                    const gName = dropZone.dataset.group;
                    draggedIdsGlobal.forEach(did => { for (const g in customGroups) customGroups[g].ids = customGroups[g].ids.filter(id => id !== did); if (!customGroups[gName].ids.includes(did)) customGroups[gName].ids.push(did); });
                    renderMyLayers(); markDirty(); hint(`Layer(s) added to "${gName}"`);
                } else if (dropZone && dropZone.id === 'ungrouped-zone') {
                    draggedIdsGlobal.forEach(did => { for (const g in customGroups) customGroups[g].ids = customGroups[g].ids.filter(id => id !== did); });
                    renderMyLayers(); markDirty(); hint('Layer(s) moved to Ungrouped');
                } else if (dropZone && dropZone.classList.contains('layer-card')) {
                    const targetId = parseInt(dropZone.dataset.id, 10);
                    draggedIdsGlobal.forEach(did => { if (did !== targetId && !isNaN(did)) reorderFeatures(did, targetId); });
                }
                draggedIdsGlobal = [];
            });
        });
        ...
        function circleCoords(c, edge) {
            ...
            for (let i = 0; i <= 64; i++) { const a = (i / 64) * 2 * Math.PI; coords.push([c[0] + (r / (111320 * Math.cos(c[1]*Math.PI/180))) * Math.cos(a), c[1] + (r / 111320) * Math.sin(a)]); }
            return { coords: [coords], r };
        }
        function pointInPolygon(point, vs) {
            const x = point[0], y = point[1];
            let inside = false;
            for (let i = 0, j = vs.length - 1; i < vs.length; j = i++) { const intersect = ((vs[i][1] > y) !== (vs[j][1] > y)) && (x < (vs[j][0] - vs[i][0]) * (y - vs[i][1]) / (vs[j][1] - vs[i][1]) + vs[i][0]); if (intersect) inside = !inside; }
            return inside;
        }
        ...
        function fetchMultiPointRoute(pts, updateId = null, mode = null) {
            ...
            fetch(`https://router.project-osrm.org/route/v1/${profile}/${pts.map(p => `${p[0]},${p[1]}`).join(';')}?overview=full&geometries=geojson&steps=true`)
            .then(r => r.json())
            .then(j => {
                if (j.routes && j.routes[0]) {
                    const dist = j.routes[0].distance, dur = j.routes[0].duration;
                    ...
                    if (updateId) {
                        ...
                        if (f) {
                            f.geometry = j.routes[0].geometry; f.props.waypoints = pts; f.props.description = desc; f.props.routeMode = profile; f.props.routingFailed = false; f.props.metadata = { distance: dist, duration: dur };
                            syncDraw(); renderMyLayers(); markDirty();
                            if (selectedId === updateId) { $('eRouteStats').textContent = desc; renderWaypointEditor(f); }
                        }
                    } else {
                        addFeatureRecord('route', j.routes[0].geometry, { color: currentRouteColor, borderColor: currentRouteColor, width: 4, borderOpacity: 0.9, description: desc, routeMode: profile, waypoints: pts, routingFailed: false, metadata: { distance: dist, duration: dur }, showLabel: true });
                    }
                ...
                if (updateId) {
                    const f = features.find(x => x.id === updateId);
                    if (f) { f.geometry = { type: 'LineString', coordinates: pts }; f.props.waypoints = pts; f.props.routingFailed = true; f.props.description = "Routing unavailable"; syncDraw(); markDirty(); }
                } else { addFeatureRecord('route', { type: 'LineString', coordinates: pts }, { color: currentRouteColor, borderColor: currentRouteColor, width: 3, borderOpacity: 0.8, routingFailed: true, waypoints: pts, description: "Routing unavailable", routeMode: profile, showLabel: true }); }
        ...
        function addFeatureRecord(kind, geometry, customProps = {}, targetGroup = null, explicitName = null) {
            ...
            const feat = { id: newId, name: assignedName, kind: kind, geometry: geometry, props: { color: kind === 'route' ? (customProps.borderColor || currentRouteColor) : '#e8b84a', borderColor: kind === 'route' ? (customProps.borderColor || currentRouteColor) : '#e8b84a', borderOpacity: 0.9, width: 3, fillColor: '#e8b84a', fillOpacity: 0.35, showLabel: False, labelPos: 'center', iconSize: markerIconSize, visible: 1, rotation: 0, attributes: { name: assignedName, description: kind === 'route' ? (customProps.description || '') : '' }, ...customProps } };
            features.push(feat);
        ...
        function showFeaturePopup(f, clickLngLat) {
            ...
            if (!popupCoords) {
                ...
                popupCoords = bnd ? [(bnd[0][0] + bnd[1][0]) / 2, (bnd[0][1] + bnd[1][1]) / 2] : [120.9842, 14.5995];
            }
            ...
            if (f.props.attrRows && f.props.attrRows.length > 0 && f.props.attrTypes) {
                for (const row of f.props.attrRows) {
                    for (const col in f.props.attrTypes) {
                        if (f.props.attrTypes[col] === 'image' && row[col] && row[col].startsWith('data:image')) { primaryImageHtml = `<img src="${row[col]}" style="width:100%; max-height:160px; object-fit:cover; border-radius:8px; margin-bottom:8px; display:block; border:1px solid rgba(255,255,255,0.15);">`; break; }
                    }
            ...
            if (f.props.osmTags && Object.keys(f.props.osmTags).length > 0) {
                ...
                for (const k in f.props.attributes) { const val = f.props.attributes[k]; if (!(val && typeof val === 'string' && val.startsWith('data:image'))) tableRowsHtml += `<tr><th>${k}</th><td>${val || '-'}</td></tr>`; }
            }
            if (!tableRowsHtml) tableRowsHtml = `<tr><th>name</th><td>${f.name}</td></tr><tr><th>type</th><td>${f.kind}</td></tr>${f.props.description ? `<tr><th>description</th><td>${f.props.description}</td></tr>` : ''}`;
            
            new maplibregl.Popup({ maxWidth: '320px' }).setLngLat(popupCoords).setHTML(`
                <div style="font-weight:700; margin-bottom:6px; color:var(--accent); font-size:13px;">${f.name}</div>
                ${primaryImageHtml}
                <table class="tag-table">${tableRowsHtml}</table>
            `).addTo(map);
        }

        function populateTradeAreaCheckboxes() {
            $('poiCategoryCheckboxes').innerHTML = Object.entries(POI_CONFIG).map(([category, items]) => `
                <div style="font-weight:600; font-size:11px; margin-top:4px;">${category}</div>
                <div class="trade-area-poi-row">${items.map(([label, tag]) => `<label> <input type="checkbox" class="poi-cat-check" data-cat="${category}" data-tag="${tag}" data-label="${label}" style="accent-color:var(--primary);"> ${label}</label>`).join('')}</div>
            `).join('');
        }

        $('btnOpenTradeAreaPopup').onclick = () => {
            closeFloatingCards(); $('trade-area-modal').style.display = 'flex';
            $('tradePolygonSelect').innerHTML = '<option value="">-- Choose --</option>' + features.filter(f => ['polygon','rectangle','circle'].includes(f.kind)).map(p => `<option value="${p.id}">${p.name}</option>`).join('');
        };
        $('closeTradeAreaBtn').onclick = () => $('trade-area-modal').style.display = 'none';

        async function robustOverpassFetch(query, timeout = 90) {
            for (const endpoint of ["https://overpass-api.de/api/interpreter", "https://overpass.kumi.systems/api/interpreter", "https://overpass.openstreetmap.fr/api/interpreter"]) {
                for (let retries = 5, delay = 1000; retries > 0; retries--) {
                    try {
                    ...
                    } catch (err) { if (retries === 1) break; await new Promise(r => setTimeout(r, delay + Math.random() * 500)); delay *= 2; }
                }
            }
            return null;
        }
        ...
        $('btnScanTradeArea').onclick = async () => {
            ...
            if (!targetPoly) return hint('Please select a target polygon first.');

            let selectedTags = Array.from(document.querySelectorAll('.poi-cat-check:checked')).map(cb => cb.dataset.tag);
            const customSearch = $('customPoiSearchInput').value.trim();
            if (customSearch) selectedTags.push(customSearch.includes('=') ? `"${customSearch.split('=')[0]}"="${customSearch.split('=')[1]}"` : `"amenity"~"${customSearch}",i`);
            if (!selectedTags.length) return hint('Please select at least one category or enter a custom POI tag.');

            const bnd = calcBounds(targetPoly), bbox = `${bnd[0][1]},${bnd[0][0]},${bnd[1][1]},${bnd[1][0]}`;
            const queryParts = selectedTags.map(rawTag => {
                if (rawTag.includes('~')) { const [k, v] = rawTag.split('~'); return `node["${k.replace(/"/g, '')}"~"${v.replace(/"/g, '').replace(',i', '')}",i](${bbox});way["${k.replace(/"/g, '')}"~"${v.replace(/"/g, '').replace(',i', '')}",i](${bbox});`; }
                if (rawTag.includes('=')) { const [k, v] = rawTag.split('='); return `node["${k.replace(/"/g, '')}"="${v.replace(/"/g, '')}"](${bbox});way["${k.replace(/"/g, '')}"="${v.replace(/"/g, '')}"](${bbox});`; }
                return '';
            }).join('');
            hint('Scanning POIs with robust fetch…'); $('tradeResults').innerHTML = '<div style="color:#d9b451;">Querying spatial features…</div>';
            const data = await robustOverpassFetch(`[out:json][timeout:25];(${queryParts});out center 100;`, 90);
            if (!data) return hint('Query failed.');
            const filtered = (data.elements || []).filter(el => { const lat = el.lat || (el.center && el.center.lat), lon = el.lon || (el.center && el.center.lon); return lat && lon && pointInPolygon([lon, lat], targetPoly.geometry.coordinates[0]); });
            if (!filtered.length) { $('tradeResults').innerHTML = '<div style="color:#8b949e;">No matching POIs inside this area.</div>'; return hint('Scan complete: 0 POIs.'); }
            ...
            filtered.forEach(el => {
                ...
                addFeatureRecord('marker', { type: 'Point', coordinates: [el.lon || el.center.lon, el.lat || el.center.lat] }, { shape: 'pin', color: '#1e40af', iconSize: 0.85, iconKey: getIconKey('pin', '#1e40af'), osmTags: el.tags || { name: poiName, type: 'custom' } }, "Trade Area Scan", poiName);
            });
            $('tradeResults').innerHTML = `<div style="font-weight:700; color:var(--text-main); margin-bottom:4px;">Grouped ${filtered.length} POIs:</div>` + Object.entries(counts).map(([k, v]) => `<div class="poi-badge"><span>${k}</span> <span style="font-weight:700; color:var(--accent);">${v}</span></div>`).join('');
            hint(`Added ${filtered.length} POIs to "Trade Area Scan"!`);
        };

        $('customQueryToggle').onclick = () => { const body = $('customQueryBody'), toggleIcon = $('customQueryToggle').querySelector('span:last-child'); body.style.display = body.style.display === 'none' ? 'block' : 'none'; toggleIcon.textContent = body.style.display === 'block' ? '▾' : '▸'; };

        $('btnRunOverpass').onclick = async () => {
            const ql = $('overpassQueryInput').value.trim(); if (!ql) return hint('Please enter an Overpass QL query');
            hint('Running custom Overpass query…');
            const data = await robustOverpassFetch(ql, 90);
            if (!data || !data.elements.length) return hint('Query failed or returned no results');
            data.elements.forEach(el => {
                if (el.type === 'node') addFeatureRecord('marker', { type: 'Point', coordinates: [el.lon, el.lat] }, { shape: 'pin', color: '#1e40af', iconSize: 0.9, iconKey: getIconKey('pin', '#1e40af'), osmTags: el.tags || {} });
                    ...
                    if ($('overpassResultType').value === 'marker' && el.center) addFeatureRecord('marker', { type: 'Point', coordinates: [el.center.lon, el.center.lat] }, { shape: 'pin', color: '#1e40af', iconSize: 0.9, iconKey: getIconKey('pin', '#1e40af'), osmTags: el.tags || {} });
                    else if ($('overpassResultType').value === 'polygon') { if (el.geometry.type === 'Polygon') addFeatureRecord('polygon', el.geometry, {}); else if (el.geometry.type === 'LineString') addFeatureRecord('polyline', el.geometry, {}); }
                }
            });
            hint(`Added ${data.elements.length} features from query`);
        };

        let boundaryResults = [];
        $('boundarySearchInput').addEventListener('input', debounce(async () => {
            const q = $('boundarySearchInput').value.trim();
            if (q.length < 3) { $('boundaryAutocompleteList').style.display = 'none'; return; }
            try {
                const data = await (await fetch(`https://nominatim.openstreetmap.org/search?format=json&polygon_geojson=1&limit=5&q=${encodeURIComponent(q)}`)).json();
                boundaryResults = data;
                if (!data.length) { $('boundaryAutocompleteList').style.display = 'none'; return; }
                $('boundaryAutocompleteList').innerHTML = data.map((r, idx) => `<div class="autocomplete-item" data-index="${idx}"><div style="font-weight:600;">${r.display_name}</div><div style="font-size:10px; color:#8b949e;">${r.type}</div></div>`).join('');
                $('boundaryAutocompleteList').style.display = 'block';
                $('boundaryAutocompleteList').querySelectorAll('.autocomplete-item').forEach(item => {
                    item.onclick = () => {
                        const selected = boundaryResults[parseInt(item.dataset.index, 10)];
                        $('boundarySearchInput').value = selected.display_name; $('boundaryAutocompleteList').style.display = 'none';
                        if (selected.geojson) {
                            addFeatureRecord('polygon', selected.geojson, { borderColor: '#ff1e1e', borderOpacity: 1.0, width: 3, fillColor: '#ff1e1e', fillOpacity: 0.15, showLabel: true }, null, `${selected.display_name} Boundary`);
                            if (selected.boundingbox) map.fitBounds([[parseFloat(selected.boundingbox[2]), parseFloat(selected.boundingbox[0])], [parseFloat(selected.boundingbox[3]), parseFloat(selected.boundingbox[1])]], { padding: 60 });
                            hint(`${selected.display_name} boundary added!`);
                        }
                    };
                });
            } catch(e) {}
        ...
        document.addEventListener('click', e => { if (!e.target.closest('.bound-select-row')) $('boundaryAutocompleteList').style.display = 'none'; if (!e.target.closest('#map-context-menu')) $('map-context-menu').style.display = 'none'; });
        ...
        map.on('contextmenu', e => {
            ...
            ['ctx-edit', 'ctx-datatable', 'ctx-delete'].forEach(id => $(id).style.display = ctxFeatureId ? 'flex' : 'none');
            document.querySelectorAll('#map-context-menu .ctx-item[id^="ctx-bring"], #map-context-menu .ctx-item[id^="ctx-send"]').forEach(el => el.style.display = ctxFeatureId ? 'flex' : 'none');
            const menu = $('map-context-menu');
            menu.style.left = Math.min(e.point.x, window.innerWidth - 220) + 'px';
            menu.style.top = Math.min(e.point.y, window.innerHeight - 180) + 'px';
            menu.style.display = 'block';
        });

        map.on('movestart', () => $('map-context-menu').style.display = 'none');

        $('ctx-edit').onclick = (e) => { e.stopPropagation(); if (ctxFeatureId) { editMode = true; selectedId = ctxFeatureId; openShapeEditor(ctxFeatureId); syncVertexHandles(); hint('Edit Mode active: Drag, edit vertices, or rotate via gold handle'); } $('map-context-menu').style.display = 'none'; };
        ...
        function reorderLayer(featureId, action) {
            ...
            else if (action === 'forward') features.splice(Math.min(index + 1, features.length), 0, item);
            else if (action === 'backward') features.splice(Math.max(index - 1, 0), 0, item);
            syncDraw(); renderMyLayers(); markDirty();
        }
        $('ctx-bring-front').onclick = (e) => { e.stopPropagation(); if (ctxFeatureId) reorderLayer(ctxFeatureId, 'front'); $('map-context-menu').style.display = 'none'; };
        $('ctx-bring-forward').onclick = (e) => { e.stopPropagation(); if (ctxFeatureId) reorderLayer(ctxFeatureId, 'forward'); $('map-context-menu').style.display = 'none'; };
        $('ctx-send-backward').onclick = (e) => { e.stopPropagation(); if (ctxFeatureId) reorderLayer(ctxFeatureId, 'backward'); $('map-context-menu').style.display = 'none'; };
        $('ctx-send-back').onclick = (e) => { e.stopPropagation(); if (ctxFeatureId) reorderLayer(ctxFeatureId, 'back'); $('map-context-menu').style.display = 'none'; };
        ...
        $('searchInput').addEventListener('input', debounce(async () => {
            ...
            try {
                const data = await (await fetch(`https://nominatim.openstreetmap.org/search?format=json&limit=5&q=${encodeURIComponent(q)}`)).json();
                searchResults = data;
                if (!data.length) return $('searchResultsList').innerHTML = '';
                $('searchResultsList').innerHTML = data.map((r, idx) => `<div class="search-result-item" data-index="${idx}"><svg class="search-result-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 21s-7-6-7-11a7 7 0 0 1 14 0c0 5-7 11-7 11z"></path><circle cx="12" cy="10" r="2.5"></circle></svg><div><div style="font-weight:600;">${r.name || r.display_name}</div><div style="font-size:11px; color:#5f6368;">${r.display_name}</div></div></div>`).join('');
                $('searchResultsList').querySelectorAll('.search-result-item').forEach(item => {
                    item.onclick = () => {
                        const selected = searchResults[parseInt(item.dataset.index, 10)];
                        ...
                        const el = document.createElement('div'); el.style.cssText = 'width:20px;height:20px;background-color:var(--accent);border-radius:50%;border:2px solid #ffffff;box-shadow:0 0 12px rgba(56, 189, 248, 0.9);transition:opacity 1s ease;';
        ...
        document.querySelectorAll('.tool').forEach(btn => {
            btn.onclick = () => {
                ...
                if (activeTool === t) { resetActiveTools(); closeFloatingCards(); }
                    ...
                    document.querySelectorAll('.tool').forEach(b => b.classList.remove('primary-active'));
                    editMode = false; syncVertexHandles(); closeFloatingCards(); activeTool = t; btn.classList.add('primary-active'); draft = []; renderDraft();
                    ...
                    hint(t === 'polyline' ? 'Click points · Double-click or click last point to finish' : t === 'polygon' ? 'Click vertices · Double-click or click origin to close' : t === 'rectangle' ? 'Click corner 1, then click opposite corner' : t === 'circle' ? 'Click center, then outer edge' : '');
                }
            };
        });
        ...
        map.on('mousemove', e => {
            ...
            if (isDragging && dragFeatureId) {
                ...
                if (!f) return;
                const translateCoords = coords => typeof coords[0] === 'number' ? [coords[0] + dx, coords[1] + dy] : coords.map(translateCoords);
            ...
            if (isDraggingRotation && rotatingPolyId != null) {
                ...
                if (f && rotCenter) {
                    ...
                    rotateGeometry(f, currentAngle - rotStartAngle, rotCenter); rotStartAngle = currentAngle; syncDraw();
                }
            }
            if (isDraggingVertex && draggedPolyId != null) {
                const f = features.find(x => x.id === draggedPolyId);
                if (f) {
                    if (isRadiusHandle && f.kind === 'circle') { f.props.radiusMeters = haversineDist(f.props.centerCoord, cursorLL); f.geometry.coordinates = circleCoords(f.props.centerCoord, cursorLL).coords; }
                    else if (['polygon','rectangle'].includes(f.kind) && f.geometry.coordinates[0]) { f.geometry.coordinates[0][draggedVertexIdx] = cursorLL; if (draggedVertexIdx === 0) f.geometry.coordinates[0][f.geometry.coordinates[0].length - 1] = cursorLL; }
                    else if (f.kind === 'polyline' && f.geometry.coordinates) { f.geometry.coordinates[draggedVertexIdx] = cursorLL; }
                    else if (f.kind === 'route' && f.props.waypoints) { f.props.waypoints[draggedVertexIdx] = cursorLL; clearTimeout(rerouteTimeout); rerouteTimeout = setTimeout(() => fetchMultiPointRoute(f.props.waypoints, f.id, f.props.routeMode), 300); }
                    syncDraw();
                }
            }
        });
        ...
        map.on('click', e => {
            ...
            if (activeTool === 'marker') {
                const feat = addFeatureRecord('marker', { type: 'Point', coordinates: ll }, { shape: markerShape, color: markerColor, iconSize: markerIconSize, iconKey: customMarkerImageKey ? customMarkerImageKey : getIconKey(markerShape, markerColor) });
                resetActiveTools(); closeFloatingCards(); showFeaturePopup(feat, ll);
            } else if (activeTool === 'textbox') {
                const feat = addFeatureRecord('text', { type: 'Point', coordinates: ll }, { text: $('tContent').value || 'Label', fontSize: parseInt($('tSize').value, 10), color: $('tColorCtrl').dataset.val || '#d9b451', opacity: parseFloat($('tOp').value) });
                resetActiveTools(); closeFloatingCards(); showFeaturePopup(feat, ll);
            } else if (activeTool === 'polyline') {
                if (draft.length >= 2 && Math.hypot(map.project(ll).x - map.project(draft[draft.length - 1]).x, map.project(ll).y - map.project(draft[draft.length - 1]).y) < 18) {
                    const feat = addFeatureRecord('polyline', { type: 'LineString', coordinates: draft }); resetActiveTools(); showFeaturePopup(feat, ll); return;
                }
                draft.push(ll);
            } else if (activeTool === 'polygon') {
                if (draft.length >= 3) {
                    for (const pt of draft) {
                        if (Math.hypot(map.project(ll).x - map.project(pt).x, map.project(ll).y - map.project(pt).y) < 18) {
                            const feat = addFeatureRecord('polygon', { type: 'Polygon', coordinates: [[...draft, draft[0]]] }); resetActiveTools(); showFeaturePopup(feat, ll); return;
                        }
                    }
                }
                draft.push(ll);
                ...
                draft.push(ll);
                if (draft.length === 2) { const feat = addFeatureRecord('rectangle', { type: 'Polygon', coordinates: rectCoords(draft[0], draft[1]) }); resetActiveTools(); showFeaturePopup(feat, ll); }
            } else if (activeTool === 'circle') {
                draft.push(ll);
                if (draft.length === 2) { const { coords, r } = circleCoords(draft[0], draft[1]); const feat = addFeatureRecord('circle', { type: 'Polygon', coordinates: coords }, { centerCoord: draft[0], radiusMeters: r }); resetActiveTools(); showFeaturePopup(feat, ll); }
            } else if (activeTool === 'route') {
                if (draft.length >= 2 && Math.hypot(map.project(ll).x - map.project(draft[draft.length - 1]).x, map.project(ll).y - map.project(draft[draft.length - 1]).y) < 22) { fetchMultiPointRoute(draft); resetActiveTools(); return; }
                draft.push(ll);
            }
            renderDraft();
        });
        ...
        map.on('dblclick', e => {
            if (activeTool === 'route' && $('rAutoFinish') && $('rAutoFinish').checked && draft.length >= 2) { e.preventDefault(); fetchMultiPointRoute(draft); resetActiveTools(); return; }
            if (editMode) {
                ...
                if (fs.length && fs[0].properties.id != null) {
                    ...
                    if (f && f.kind === 'route' && f.props.waypoints) {
                        ...
                        for (let i = 0; i < f.props.waypoints.length - 1; i++) {
                            const d = haversineDist(f.props.waypoints[i], ll) + haversineDist(ll, f.props.waypoints[i+1]);
                            if (d < bestDist) { bestDist = d; bestIdx = i + 1; }
                        }
                        if (bestIdx !== -1) { f.props.waypoints.splice(bestIdx, 0, ll); fetchMultiPointRoute(f.props.waypoints, f.id, f.props.routeMode); hint('Waypoint inserted'); }
                    }
                }
            }
        });
            ...
            if (editMode) {
                ...
                if (vHits.length && vHits[0].properties.polyId != null) {
                    ...
                    if (prop.isRotHandle) {
                        ...
                        const b = calcBounds(features.find(x => x.id === rotatingPolyId));
                    ...
                    isDraggingVertex = true; draggedPolyId = parseInt(prop.polyId, 10); draggedVertexIdx = prop.vIdx != null ? parseInt(prop.vIdx, 10) : -1; isRadiusHandle = !!prop.isRadiusHandle; map.dragPan.disable(); return;
                }
                const fs = map.queryRenderedFeatures(e.point, { layers: ['draw-fill','draw-line','draw-outline','draw-marker','draw-text'] });
                if (fs.length && fs[0].properties.id != null) {
                    isDragging = true; dragFeatureId = parseInt(fs[0].properties.id, 10); dragStartCoord = [e.lngLat.lng, e.lngLat.lat];
                    const f = features.find(x => x.id === dragFeatureId); if (f) dragOriginalCoords = JSON.parse(JSON.stringify(f.geometry.coordinates)); map.dragPan.disable();
                }
            }
        });
        ...
        map.on('mouseup', () => {
            ...
            if (isDraggingVertex) {
                if (draggedPolyId != null) { const f = features.find(x => x.id === draggedPolyId); if (f && f.kind === 'route' && f.props.waypoints) fetchMultiPointRoute(f.props.waypoints, f.id, f.props.routeMode); }
        ...
        function openShapeEditor(id) {
            ...
            selectedId = id; closeFloatingCards();
            ...
            $('eMarkerSizeRow').style.display = f.kind === 'marker' ? 'flex' : 'none'; if (f.kind === 'marker') $('eMarkerSize').value = f.props.iconSize || 0.9;
            $('eTextRow').style.display = f.kind === 'text' ? 'flex' : 'none'; $('eFontSizeRow').style.display = f.kind === 'text' ? 'flex' : 'none';
            if (f.kind === 'text') { $('eTextVal').value = f.props.text || ''; $('eFontSize').value = f.props.fontSize || 16; }
            $('routeEditorControls').style.display = f.kind === 'route' ? 'block' : 'none';
            if (f.kind === 'route') { $('eRouteMode').value = f.props.routeMode || 'driving'; $('eRouteStats').textContent = f.props.description || '-'; renderWaypointEditor(f); }
        ...
        window.removeWaypoint = function(featId, idx) { const f = features.find(x => x.id === featId); if (f && f.props.waypoints && f.props.waypoints.length > 2) { f.props.waypoints.splice(idx, 1); fetchMultiPointRoute(f.props.waypoints, f.id, f.props.routeMode); } };
        ...
        $('btnSelectAllGlobal').onclick = () => { if (selectedLayerIds.size === features.length && features.length > 0) selectedLayerIds.clear(); else features.forEach(f => selectedLayerIds.add(f.id)); renderMyLayers(); };
        $('btnDeleteSelectedGlobal').onclick = () => {
            if (!selectedLayerIds.size) return hint('Select at least one layer first');
            ...
            selectedLayerIds.clear(); syncDraw(); renderMyLayers(); markDirty(); hint('Selected layers deleted');
        };
        $('btnHideSelected').onclick = () => {
            if (!selectedLayerIds.size) return hint('Select at least one layer first');
            const newVis = Array.from(selectedLayerIds).every(id => { const f = features.find(x => x.id === id); return f && f.props.visible === 0; }) ? 1 : 0;
            selectedLayerIds.forEach(id => { const f = features.find(x => x.id === id); if (f) f.props.visible = newVis; });
            syncDraw(); renderMyLayers(); markDirty(); hint(newVis ? 'Layers shown' : 'Layers hidden');
        };
        ...
        function renderLayerCardHtml(f) {
            ...
            return `
                <div class="layer-card ${isSelected ? 'selected' : ''}" draggable="true" data-id="${f.id}">
                    <div class="layer-card-top">
                        <input type="checkbox" class="layer-select-check" data-id="${f.id}" ${isSelected ? 'checked' : ''} style="width:14px;height:14px;accent-color:var(--primary);cursor:pointer;">
                        <input class="layer-name-input" data-id="${f.id}" value="${f.name}" title="Click to rename">
                        <button class="card-btn" data-act="table" data-id="${f.id}" title="View/Edit Attributes"><svg viewBox="0 0 24 24" width="12" height="12" fill="none" stroke="currentColor" stroke-width="2"><rect x="3" y="3" width="18" height="18" rx="2" ry="2"></rect><line x1="3" y1="9" x2="21" y2="9"></line><line x1="9" y1="21" x2="9" y2="9"></line></svg></button>
                    ...
                    <div style="display:flex; justify-content:space-between; align-items:center; font-size:10px; color:#768390; padding:0 4px; gap:4px;">
                        ...
                        <span style="display:flex; align-items:center; gap:4px;">
                            <label style="display:flex; align-items:center; gap:3px; cursor:pointer;"><input type="checkbox" data-act="labelToggle" data-id="${f.id}" ${f.props.showLabel ? 'checked' : ''} style="width:10px;height:10px;accent-color:var(--primary);"> Label</label>
                            <select data-act="labelPos" data-id="${f.id}" title="Label Position" style="font-size:9px; background:rgba(0,0,0,0.4); color:var(--text-muted); border:1px solid var(--border-color); border-radius:4px; padding:1px 2px;">${['center','top','bottom','left','right'].map(p => `<option value="${p}" ${(f.props.labelPos || 'center') === p ? 'selected' : ''}>${p}</option>`).join('')}</select>
                        </span>
                    </div>
                </div>
            `;
        ...
        function renderMyLayers() {
            ...
            $('tradePolygonSelect').innerHTML = '<option value="">-- Choose --</option>' + features.filter(f => ['polygon', 'rectangle', 'circle'].includes(f.kind)).map(p => `<option value="${p.id}">${p.name}</option>`).join('');
            if (!features.length && !Object.keys(customGroups).length) { $('my-layers-list').innerHTML = '<div style="font-size:12px; color:#768390; padding:6px 0;">No drawings yet. Use the tools to create shapes.</div>'; return; }
            let html = ''; const groupedIds = new Set();
            for (const gName in customGroups) {
                const grp = customGroups[gName], groupFeats = features.filter(f => grp.ids.includes(f.id)); grp.ids.forEach(id => groupedIds.add(id));
                html += `
                    <div class="group-container" data-group="${gName}">
                        <div class="group-header" data-group="${gName}">
                            <div style="display:flex; align-items:center; gap:6px;">
                                <button class="card-btn" data-act="groupToggleCollapse" data-group="${gName}"><svg viewBox="0 0 24 24" width="10" height="10" fill="none" stroke="currentColor" stroke-width="2"><polyline points="${grp.collapsed ? '9 18 15 12 9 6' : '6 9 12 15 18 9'}"></polyline></svg></button>
                                <input class="group-title-input" data-oldname="${gName}" value="${gName}" title="Click to rename Group">
                            </div>
                            <div style="display:flex; align-items:center; gap:2px;">
                                <button class="card-btn" data-act="groupSelectAll" data-group="${gName}" title="Select All in Group"><svg viewBox="0 0 24 24" width="12" height="12" fill="none" stroke="currentColor" stroke-width="2"><path d="M9 11l3 3L22 4"></path><path d="M21 12v7a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h11"></path></svg></button>
                                <button class="card-btn" data-act="groupStyle" data-group="${gName}" title="Style Group"><svg viewBox="0 0 24 24" width="12" height="12" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 20h9"></path><path d="M16.5 3.5a2.121 2.121 0 0 1 3 3L7 19l-4 1 1-4L16.5 3.5z"></path></svg></button>
                                <button class="card-btn" data-act="groupEye" data-group="${gName}" title="Toggle Group"><svg viewBox="0 0 24 24" width="12" height="12" fill="none" stroke="currentColor" stroke-width="2"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"></path><circle cx="12" cy="12" r="3"></circle></svg></button>
                        ...
                        <div class="group-styling-panel" data-group="${gName}">
                            ...
                            <div class="f-row" style="margin-bottom:4px;"> <span style="font-size:11px;">Shape</span> <select class="grp-style-shape" style="flex:1; font-size:10px;"><option value="">-- No Change --</option><option value="pin">Pin</option><option value="star">Star</option><option value="circle">Circle</option><option value="square">Square</option><option value="flag">Flag</option><option value="heart">Heart</option><option value="pinball">Pinball</option></select> </div>
                        ...
                        <div class="group-items ${grp.collapsed ? 'hidden' : ''}">${groupFeats.length ? groupFeats.map(f => renderLayerCardHtml(f)).join('') : '<div style="font-size:10px; color:#768390; padding:4px;">Empty group — drag layers here</div>'}</div>
                    </div>
                `;
            }
            const looseFeats = features.filter(f => !groupedIds.has(f.id));
            html += `<div id="ungrouped-zone">${looseFeats.length ? `<div style="font-size:11px; font-weight:700; color:var(--text-muted); margin-top:8px; display:flex; justify-content:space-between; align-items:center;"> <span>Ungrouped Layers</span> <button class="card-btn" id="btnSelectAllUngrouped" title="Select All Ungrouped"> <svg viewBox="0 0 24 24" width="12" height="12" fill="none" stroke="currentColor" stroke-width="2"><path d="M9 11l3 3L22 4"></path><path d="M21 12v7a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h11"></path></svg> </button> </div>` + looseFeats.slice().reverse().map(f => renderLayerCardHtml(f)).join('') : ''}</div>`;
            $('my-layers-list').innerHTML = html;
        }
        ...
        function openAttributeTable(featureId) {
            ...
            if (!f.props.attributes) f.props.attributes = { name: f.name || '', description: f.props.description || '' };
            if (!f.props.attrTypes) f.props.attrTypes = { name: 'text', description: 'text' };
            if (!f.props.attrRows) f.props.attrRows = [{ ...f.props.attributes }];
            renderAttributeTable(f);
        }

        function renderAttributeTable(f) {
            const types = f.props.attrTypes || { name: 'text', description: 'text' }, cols = Object.keys(types).filter(k => k !== 'name'), rows = f.props.attrRows || [{ ...f.props.attributes }];
            $('attrTableHeader').innerHTML = `<tr>` + cols.map(c => `<th><div style="display:flex; justify-content:space-between; align-items:center; gap:4px;"><span>${I encountered an error doing what you asked. Could you try again?
