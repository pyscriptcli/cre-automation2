...
/* Small, square, thin-bordered color inputs */
input[type="color"] {
    width: 22px;
    height: 22px;
    padding: 0;
...
<div id="top-toolbar-bar">
    ...
    <button class="tb-btn" id="btn-export-dialog" title="Export Map as PNG">
    ...
    <div class="panel-content">
        ...
        <div class="acc-item">
            ...
            <div class="acc-body hidden" id="body-trade-area">
                <div style="font-size:11px; color:#8b949e;">Click this header to open the Trade Area Analysis tool.</div>
            </div>
        </div>
        <div class="acc-item">
...
<div id="trade-area-modal" class="modal-scrim">
    <div class="float-card open" style="width:500px; max-width:90vw; max-height:85vh; padding:16px;">
        ...
        <div style="font-weight:600; font-size:11px; color:#768390; margin-top:8px;">CUSTOM POI SEARCH</div>
        <div style="display:flex; gap:6px;">
            <input type="text" id="customPoiSearchInput" placeholder="e.g. Jollibee, Starbucks..." style="flex:1; background:rgba(0,0,0,0.4); border:1px solid rgba(255,255,255,0.12); color:#f0f6fc; padding:6px 8px; border-radius:6px; font-size:11px;" />
            <button id="btnRunCustomPoiSearch" class="trade-btn" style="padding:6px 12px;">Search</button>
        </div>

        <div style="font-weight:600; font-size:11px; color:#768390; margin-top:12px;">POI CATEGORIES</div>
        <div id="poiCategoryCheckboxes" style="max-height:300px; overflow-y:auto; display:flex; flex-direction:column; gap:12px; padding: 8px; background: rgba(0,0,0,0.2); border-radius: 8px;">
        </div>
        
        <button class="trade-btn" id="btnScanTradeArea" style="margin-top:8px;">Scan Selected POIs</button>
        ...
        <div id="customQueryBody" style="display:none; margin-top:8px;">
            <textarea id="overpassQueryInput" rows="5" style="width:100%; background:rgba(0,0,0,0.4); color:#f0f6fc; border:1px solid rgba(255,255,255,0.12); border-radius:8px; padding:8px; font-size:12px; font-family: monospace;"></textarea>
            <div class="f-row" style="margin-top:6px;"><span>Result type</span>
                <select id="overpassResultType" style="width:110px;">
                    <option value="marker">Markers</option>
                    <option value="polygon">Polygons</option>
                </select>
            </div>
            <button id="btnRunOverpass" class="trade-btn" style="margin-top:8px;">Run Custom Query</button>
        </div>
    </div>
</div>
...
$('customMarkerFileInput').onchange = function(e) {
    ...
    reader.onload = (ev) => {
        ...
        img.onload = () => {
            ...
            const imgData = c.getContext('2d').getImageData(0,0,64,64);
            try {
                if (map.hasImage(key)) map.removeImage(key);
                map.addImage(key, imgData, { pixelRatio: 1 }); // Fixed scaling
...
function populateTradeAreaCheckboxes() {
    ...
    for (const category in POI_CONFIG) {
        html += `<div style="font-weight:700; font-size:12px; color:#38bdf8; border-bottom:1px solid rgba(255,255,255,0.08); padding-bottom:4px;">${category}</div>`;
        html += '<div style="display:flex; flex-direction:column; gap:4px; padding-left:8px;">';
        POI_CONFIG[category].forEach(([label, tag], idx) => {
            html += `<label style="display:flex; align-items:center; gap:6px; font-size:11px; color:#adbac7; cursor:pointer;"><input type="checkbox" class="poi-cat-check" data-tag="${tag}" style="accent-color:#316dca;"> ${label}</label>`;
        });
        html += '</div>';
    }
...
// Intercept Trade Area accordion header to open modal directly
const tradeAreaHeader = document.querySelector('[data-target="body-trade-area"]');
if (tradeAreaHeader) {
    tradeAreaHeader.onclick = (e) => {
        e.preventDefault();
        e.stopPropagation();
        closeFloatingCards();
...
$('btnRunCustomPoiSearch').onclick = () => {
    ...
    const searchText = $('customPoiSearchInput').value.trim();
    if (!searchText) { hint('Enter a POI name to search.'); return; }
    
    const bnd = calcBounds(targetPoly);
    const bbox = `${bnd[0][1]},${bnd[0][0]},${bnd[1][1]},${bnd[1][0]}`;
    
    const safeSearch = searchText.replace(/"/g, '\\"');
    const query = `[out:json][timeout:25];(node["name"~"${safeSearch}",i](${bbox});way["name"~"${safeSearch}",i](${bbox}););out center 100;`;
    
    hint(`Searching for "${searchText}"...`);
    fetch(`https://overpass-api.de/api/interpreter?data=${encodeURIComponent(query)}`)
        ...
        .then(data => {
            ...
            if (!filtered.length) {
                hint(`No "${searchText}" found inside the polygon.`);
                return;
            }
            ...
            filtered.forEach(el => {
                const poiName = (el.tags && el.tags.name) || searchText;
                const lat = el.lat || (el.center && el.center.lat);
                const lon = el.lon || (el.center && el.center.lon);
            ...
            hint(`Added ${filtered.length} "${searchText}" locations!`);
            $('customPoiSearchInput').value = '';
        })
