...
#popup-route-settings { width: 280px; }
...
.route-directions-panel.open { display: block; }
.route-stop-item {
    display: flex;
    align-items: center;
    gap: 4px;
...
.route-stop-item.dragging { opacity: 0.4; }
.route-stop-item .stop-handle { cursor: grab; color: #768390; }
.route-stop-item .stop-remove { color: #ff7b72; cursor: pointer; }
...
.height-control-row input[type=range] { flex: 1; }
.height-display { min-width: 50px; font-weight: 700; color: #38bdf8; }
...
<div id="top-toolbar-bar">
    ...
    <button class="tb-btn tool" data-tool="polygon3d" title="Draw 3D Polygon (Height)">
        <svg viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 3l8 6-3 10H7L4 9z"></path><line x1="12" y1="3" x2="12" y2="21" stroke-dasharray="2 2"/></svg>
    </button>
    <button class="tb-btn tool" data-tool="imageoverlay" title="Place Image Overlay">
        <svg viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="currentColor" stroke-width="2"><rect x="3" y="3" width="18" height="18" rx="2" ry="2"></rect><circle cx="8.5" cy="8.5" r="1.5"></circle><path d="M21 15l-5-5L5 21"></path></svg>
    </button>
    <div class="tb-sep"></div>
    ...
    <div class="panel-content">
        <div style="margin-bottom: 8px; padding-bottom: 12px; border-bottom: 1px solid rgba(255,255,255,0.08);">
            <button id="btn-import" class="trade-btn" style="width:100%; display:flex; justify-content:center; align-items:center; gap:6px;">
                <svg viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="currentColor" stroke-width="2"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"></path><polyline points="17 8 12 3 7 8"></polyline><line x1="12" y1="3" x2="12" y2="15"></line></svg>
                Import Spatial Data (KML, GeoJSON, SHP, CSV, Image)
            </button>
    ...
    <div class="panel-content">
        <div class="layers-heading">
            ...
            <div style="display:flex; align-items:center; gap:4px;">
                ...
                <button class="icon-action-btn" id="btnDeleteSelected" title="Delete Selected Layers" style="color:#ff7b72;">
...
<div id="popup-marker-settings" class="float-card right-card">
    ...
    <div class="f-row"> <span>Size Mode</span>
        <select id="mSizeMode" style="width:100px;">
...
<div id="popup-route-settings" class="float-card right-card">
    ...
    <div class="f-row"><span>Draw Style</span>
        <select id="rDrawStyle" style="width:130px;">
            <option value="manual" selected>Manual Draw</option>
            <option value="directions">Directions (From/To)</option>
        </select>
    </div>
    <div id="routeDirectionsPanel" class="route-directions-panel">
        <div class="f-row"><span>From</span>
            <input type="text" id="routeFromInput" placeholder="Start location..." style="flex:1;"/>
        </div>
        <div class="f-row"><span>To</span>
            <input type="text" id="routeToInput" placeholder="End location..." style="flex:1;"/>
        </div>
        <div id="routeStopsList" style="margin-top:4px;"></div>
        <button id="btnAddRouteStop" class="trade-btn" style="font-size:10px; padding:4px;">+ Add Stop</button>
        <button id="btnComputeDirections" class="trade-btn" style="margin-top:4px; font-size:10px;">Compute Route</button>
    </div>
...
<div id="popup-shape-editor" class="float-card right-card">
    ...
    <!-- 3D Polygon Controls -->
    <div id="e3DControls" style="display:none; border-top:1px solid rgba(255,255,255,0.1); margin-top:8px; padding-top:8px;">
        <div style="font-weight:600; font-size:11px; color:#768390;">3D Settings</div>
        <div class="height-control-row">
            <span style="font-size:11px;">Height (m)</span>
            <input type="range" id="eHeight" min="0" max="200" step="1" value="20">
            <span class="height-display" id="eHeightDisplay">20</span>
        </div>
        <div class="height-control-row">
            <span style="font-size:11px;">Base Height (m)</span>
            <input type="range" id="eBaseHeight" min="0" max="200" step="1" value="0">
            <span class="height-display" id="eBaseHeightDisplay">0</span>
        </div>
        <div class="f-row">
            <span>Enable 3D</span>
            <input type="checkbox" id="eIs3D" checked style="accent-color:#316dca;">
        </div>
    </div>
    <!-- Circle Radius Display -->
    <div id="eCircleRadiusDisplay" style="display:none; font-weight:600; color:#38bdf8; font-size:12px; margin-top:4px;">
        Radius: <span id="eCircleRadiusVal">0</span> m
    </div>
    <!-- Image Overlay Controls -->
    <div id="eImageControls" style="display:none; border-top:1px solid rgba(255,255,255,0.1); margin-top:8px; padding-top:8px;">
        <div class="f-row"> <span>Opacity</span> <input type="range" id="eImageOpacity" min="0" max="1" step="0.05" value="1"></div>
        <button id="eResetImageSize" class="trade-btn" style="font-size:10px; padding:4px;">Reset Size</button>
    </div>
    <!-- Marker Controls -->
    <div id="eMarkerControls" style="display:none; border-top:1px solid rgba(255,255,255,0.1); margin-top:8px; padding-top:8px;">
        <div class="f-row"> <span>Icon Size</span> <input type="range" id="eMarkerSize" min="0.4" max="2.0" step="0.1"></div>
        <div class="f-row"> <span>Size Mode</span>
            <select id="eMarkerSizeMode" style="width:100px;">
                <option value="dynamic">Dynamic</option>
                <option value="static" selected>Static</option>
            </select>
        </div>
    ...
    <!-- Text Controls -->
    <div id="eTextControls" style="display:none; border-top:1px solid rgba(255,255,255,0.1); margin-top:8px; padding-top:8px;">
        <div class="f-row"> <span>Text</span> <input type="text" id="eTextVal" style="width:140px;"></div>
        <div class="f-row"> <span>Font Size</span> <input type="range" id="eFontSize" min="10" max="42" step="1"></div>
    </div>
    <!-- Route-specific -->
    ...
    <div class="f-row" style="margin-top:6px;"> <span>Show Label</span> <input type="checkbox" id="eShowLabel"></div>
    <div class="f-row"> <span>Label Position</span>
...
<div id="map-context-menu">
    ...
    <div class="ctx-item" id="ctx-bringfront" style="display:none;">
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="12 2 12 22"/><polyline points="7 7 12 2 17 7"/><polyline points="7 17 12 22 17 17"/></svg>
        Bring to Front
    </div>
    <div class="ctx-item" id="ctx-bringforward" style="display:none;">
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="12 2 12 22"/><polyline points="9 9 12 6 15 9"/><polyline points="9 15 12 18 15 15"/></svg>
        Bring Forward
    </div>
    <div class="ctx-item" id="ctx-sendbackward" style="display:none;">
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="12 22 12 2"/><polyline points="15 15 12 18 9 15"/><polyline points="15 9 12 6 9 9"/></svg>
        Send Backward
    </div>
    <div class="ctx-item" id="ctx-sendback" style="display:none;">
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="12 22 12 2"/><polyline points="17 17 12 22 7 17"/><polyline points="17 7 12 2 7 7"/></svg>
        Send to Back
    </div>
...
let routeDrawStyle = 'manual'; // 'manual' or 'directions'
...
function addDrawStack() {
    if (!map.getSource('draw')) {
        ...
        map.addLayer({
            ...
            layout: {
                ...
                'icon-size': ['case', ['==', ['get', 'sizeMode'], 'static'], ['coalesce', ['get', 'iconSize'], 0.9], ['coalesce', ['get', 'iconSize'], 0.9]],
    ...
    if (!map.getSource('vertex-handles')) {
        ...
        // Rotation handle as symbol with icon
        map.addLayer({
            id: 'vertex-rotation', type: 'symbol', source: 'vertex-handles',
            filter: ['==', ['get', 'isRotHandle'], true],
            layout: {
                'icon-image': 'rotation-icon',
                'icon-size': 0.5,
                'icon-allow-overlap': true
            },
            paint: {
        ...
        rotCanvas.width = 32; rotCanvas.height = 32;
        ...
        if (!map.hasImage('rotation-icon')) {
            map.addImage('rotation-icon', imgData, { pixelRatio: 2 });
        }
    }

    // For 3D polygons, add fill-extrusion layers for each feature
    features.forEach(f => {
        if ((f.kind === 'polygon' || f.kind === 'rectangle' || f.kind === 'circle') && f.props.is3D) {
            const layerId3d = `polygon-${f.id}-3d`;
            const layerId2d = `polygon-${f.id}-2d`;
            if (!map.getLayer(layerId3d)) {
                map.addLayer({
                    id: layerId3d,
                    type: 'fill-extrusion',
                    source: 'draw',
                    filter: ['all', ['==', ['get', 'id'], f.id]],
                    minzoom: 0,
                    paint: {
                        ...
                        'fill-extrusion-opacity': ['*', ['coalesce', ['get', 'fillOpacity'], 0.8], ['get', 'visible']]
                    }
                });
            }
            if (!map.getLayer(layerId2d)) {
                map.addLayer({
                    id: layerId2d,
                    type: 'fill',
                    source: 'draw',
...
function toggle3DMode(is3D) {
    // Toggle building layers
    if (map.getLayer('building-2d')) {
        map.setLayoutProperty('building-2d', 'visibility', is3D ? 'none' : 'visible');
    }
    if (map.getLayer('building-3d')) {
        map.setLayoutProperty('building-3d', 'visibility', is3D ? 'visible' : 'none');
    }
    // Toggle 3D polygon layers
    features.forEach(f => {
        if ((f.kind === 'polygon' || f.kind === 'rectangle' || f.kind === 'circle') && f.props.is3D) {
            const layerId3d = `polygon-${f.id}-3d`;
            const layerId2d = `polygon-${f.id}-2d`;
            if (map.getLayer(layerId3d)) {
                map.setLayoutProperty(layerId3d, 'visibility', is3D ? 'visible' : 'none');
            }
            if (map.getLayer(layerId2d)) {
                map.setLayoutProperty(layerId2d, 'visibility', is3D ? 'none' : 'visible');
            }
        }
    });
    vis.building2d = !is3D;
...
function syncVertexHandles() {
    ...
    if (f && f.props.visible !== 0) {
        ...
        // For 3D polygons, add height handle
        if (f.props.is3D && (f.kind === 'polygon' || f.kind === 'rectangle' || f.kind === 'circle')) {
            const b2 = calcBounds(f);
            if (b2) {
                const cx = (b2[0][0] + b2[1][0]) / 2;
                const cy = (b2[0][1] + b2[1][1]) / 2;
                const height = f.props.height || 20;
                const top = cy + height / 111320; // approximate meter to degree
                handleFeats.push({
                    type: 'Feature',
                    geometry: { type: 'Point', coordinates: [cx, top] },
                    properties: { polyId: f.id, isHeightHandle: true, height: height }
                });
            }
        }
    }
...
function renderDraft() {
    ...
    // 3D polygon draft: same as polygon but with height hint
    if (activeTool === 'polygon3d' && draft.length) {
        const pts = cursorLL ? [...draft, cursorLL] : draft;
        if (pts.length > 1) f.push(ln([...pts, pts[0]]));
    }
    // Image overlay draft: rectangle with dashed border
    if (activeTool === 'imageoverlay' && draft.length === 1 && cursorLL) {
        const rect = rectCoords(draft[0], cursorLL)[0];
        f.push({ type: 'Feature', geometry: { type: 'LineString', coordinates: rect }, properties: {} });
    }
...
function addFeatureRecord(kind, geometry, customProps = {}, targetGroup = null, explicitName = null) {
    ...
    // Ensure 3D props for polygons
    const isPoly = ['polygon', 'rectangle', 'circle'].includes(kind);
    const feat = {
        id: newId,
        name: assignedName,
        kind: kind,
        ...
        props: {
            ...
            is3D: isPoly ? (customProps.is3D || false) : false,
            height: isPoly ? (customProps.height || 20) : 0,
            baseHeight: isPoly ? (customProps.baseHeight || 0) : 0,
            ...customProps
        }
    };
    features.push(feat);
...
function showFeaturePopup(f, clickLngLat) {
    ...
    // 3D info
    if (f.props.is3D) {
        heightInfoHtml = `<tr><th>Height</th><td>${f.props.height || 0} m</td></tr>`;
        if (f.props.baseHeight) heightInfoHtml += `<tr><th>Base Height</th><td>${f.props.baseHeight} m</td></tr>`;
    }
    ...
    const html = `
        ...
        <div style="display:flex; justify-content:flex-end; gap:6px; margin-top:8px; border-top:1px solid rgba(255,255,255,0.08); padding-top:6px;">
            <button onclick="enableEditMode(${f.id});" style="background:#316dca; color:#fff; border:none; border-radius:4px; padding:3px 8px; font-size:10px; cursor:pointer;">Edit</button>
...
map.on('contextmenu', e => {
    ...
    if (ctxFeatureId) {
        ...
        $('ctx-bringfront').style.display = 'flex';
        $('ctx-bringforward').style.display = 'flex';
        $('ctx-sendbackward').style.display = 'flex';
        $('ctx-sendback').style.display = 'flex';
        $('ctx-delete').style.display = 'flex';
    } else {
        $('ctx-edit').style.display = 'none';
        $('ctx-datatable').style.display = 'none';
        $('ctx-bringfront').style.display = 'none';
        $('ctx-bringforward').style.display = 'none';
        $('ctx-sendbackward').style.display = 'none';
        $('ctx-sendback').style.display = 'none';
...
$('ctx-bringfront').onclick = (e) => { e.stopPropagation(); if (ctxFeatureId) bringToFront(ctxFeatureId); $('map-context-menu').style.display = 'none'; };
$('ctx-bringforward').onclick = (e) => { e.stopPropagation(); if (ctxFeatureId) bringForward(ctxFeatureId); $('map-context-menu').style.display = 'none'; };
$('ctx-sendbackward').onclick = (e) => { e.stopPropagation(); if (ctxFeatureId) sendBackward(ctxFeatureId); $('map-context-menu').style.display = 'none'; };
$('ctx-sendback').onclick = (e) => { e.stopPropagation(); if (ctxFeatureId) sendToBack(ctxFeatureId); $('map-context-menu').style.display = 'none'; };
...
$('rDrawStyle').onchange = e => {
    routeDrawStyle = e.target.value;
    if (routeDrawStyle === 'directions') {
        $('routeDirectionsPanel').classList.add('open');
    } else {
        $('routeDirectionsPanel').classList.remove('open');
    }
};
$('btnStartRouteDraw').onclick = () => {
    ...
    if (routeDrawStyle === 'directions') {
        // Compute from inputs
        const fromVal = $('routeFromInput').value.trim();
        const toVal = $('routeToInput').value.trim();
        if (!fromVal || !toVal) { hint('Please enter both From and To locations.'); return; }
        // Use geocoding to get coords
        Promise.all([
            fetch(`https://nominatim.openstreetmap.org/search?format=json&limit=1&q=${encodeURIComponent(fromVal)}`).then(r => r.json()),
            fetch(`https://nominatim.openstreetmap.org/search?format=json&limit=1&q=${encodeURIComponent(toVal)}`).then(r => r.json())
        ]).then(([fromRes, toRes]) => {
            if (!fromRes.length || !toRes.length) { hint('Could not find one or both locations.'); return; }
            const fromCoords = [parseFloat(fromRes[0].lon), parseFloat(fromRes[0].lat)];
            const toCoords = [parseFloat(toRes[0].lon), parseFloat(toRes[0].lat)];
            const waypoints = [fromCoords, ...routeWaypoints.map(w => w.coords), toCoords];
            fetchMultiPointRoute(waypoints);
            resetActiveTools();
        }).catch(() => hint('Geocoding failed.'));
    } else {
...
$('btnAddRouteStop').onclick = () => {
    const val = prompt('Enter stop location:');
    if (val && val.trim()) {
        // Geocode and add to waypoints
        fetch(`https://nominatim.openstreetmap.org/search?format=json&limit=1&q=${encodeURIComponent(val)}`)
            .then(r => r.json())
            .then(res => {
                if (res.length) {
                    const coords = [parseFloat(res[0].lon), parseFloat(res[0].lat)];
                    routeWaypoints.push({ label: val.trim(), coords: coords });
                    renderRouteStops();
                } else {
...
function renderRouteStops() {
    ...
    list.innerHTML = routeWaypoints.map((w, i) => `
        <div class="route-stop-item" draggable="true" data-index="${i}">
            <span class="stop-handle">⠿</span>
            <span>${w.label}</span>
            <button class="card-btn stop-remove" data-index="${i}" style="margin-left:auto;">✕</button>
        </div>
    `).join('');
    ...
    list.querySelectorAll('.route-stop-item').forEach(item => {
        ...
        item.addEventListener('drop', e => {
            ...
            if (dragIdx !== null && dragIdx !== dropIdx) {
                const [removed] = routeWaypoints.splice(dragIdx, 1);
                routeWaypoints.splice(dropIdx, 0, removed);
                renderRouteStops();
            }
        });
    });
...
$('btnComputeDirections').onclick = () => {
    // Similar to start drawing but without closing popup
    const fromVal = $('routeFromInput').value.trim();
    const toVal = $('routeToInput').value.trim();
    if (!fromVal || !toVal) { hint('Please enter both From and To locations.'); return; }
    Promise.all([
...
document.querySelectorAll('.tool').forEach(btn => {
    btn.onclick = () => {
        ...
        if (activeTool === t) {
            ...
            if (t === 'polygon3d') hint('Click vertices to draw 3D polygon (height set in editor)');
            if (t === 'rectangle') hint('Click corner 1, then click opposite corner');
            if (t === 'circle') hint('Click center, then outer edge');
            if (t === 'imageoverlay') {
                hint('Click first corner, then opposite corner to place image');
                ...
                input.onchange = (e) => {
                    ...
                    if (file) {
                        const reader = new FileReader();
                        reader.onload = (ev) => {
                            window._pendingImageData = ev.target.result;
                            hint('Image loaded. Click two corners to place.');
                        };
                        reader.readAsDataURL(file);
                    }
                };
    ...
    if (activeTool === 'marker') {
        ...
        if (draft.length >= 3) {
            ...
            for (const pt of draft) {
                ...
                if (Math.hypot(pScreen.x - vScreen.x, pScreen.y - vScreen.y) < 18) {
                    const feat = addFeatureRecord('polygon', { type: 'Polygon', coordinates: [[...draft, draft[0]]] }, { is3D: false });
                    resetActiveTools();
                    showFeaturePopup(feat, ll);
                    return;
                }
        ...
        if (draft.length >= 3) {
            ...
            for (const pt of draft) {
                const vScreen = map.project(pt);
                if (Math.hypot(pScreen.x - vScreen.x, pScreen.y - vScreen.y) < 18) {
                    const feat = addFeatureRecord('polygon', { type: 'Polygon', coordinates: [[...draft, draft[0]]] }, { is3D: true, height: 20, baseHeight: 0 });
                    resetActiveTools();
                    showFeaturePopup(feat, ll);
                    return;
                }
        ...
        draft.push(ll);
        if (draft.length === 2) {
            const feat = addFeatureRecord('rectangle', { type: 'Polygon', coordinates: rectCoords(draft[0], draft[1]) }, { is3D: false });
            resetActiveTools();
            showFeaturePopup(feat, ll);
        }
        ...
        if (draft.length === 2) {
            ...
            const feat = addFeatureRecord('circle', { type: 'Polygon', coordinates: coords }, { centerCoord: draft[0], radiusMeters: r, is3D: false });
            resetActiveTools();
            showFeaturePopup(feat, ll);
        }
        ...
        draft.push(ll);
    } else if (activeTool === 'imageoverlay') {
        draft.push(ll);
        if (draft.length === 2) {
            // Create image overlay feature
            const imageData = window._pendingImageData;
            if (!imageData) { hint('No image loaded. Please select an image first.'); resetActiveTools(); return; }
            const rect = rectCoords(draft[0], draft[1]);
            const feat = addFeatureRecord('image', { type: 'Polygon', coordinates: rect }, {
                imageData: imageData,
                opacity: 1.0,
                is3D: false
            });
            ...
            closeFloatingCards();
            showFeaturePopup(feat, ll);
            // Add image source and layer for this feature
            const imgSrc = imageData;
            const sourceId = `image-${feat.id}`;
            map.addSource(sourceId, {
                type: 'image',
                url: imgSrc,
                coordinates: rect[0] // [ [x,y], [x2,y], [x2,y2], [x,y2] ]
            });
            map.addLayer({
                id: `image-layer-${feat.id}`,
                type: 'raster',
                source: sourceId,
...
map.on('mousedown', e => {
    if (editMode) {
        const vHits = map.queryRenderedFeatures(e.point, { layers: ['vertex-points', 'vertex-rotation'] });
        ...
        // Check for height handle (circle)
        const hHits = map.queryRenderedFeatures(e.point, { layers: ['vertex-points'] });
        if (hHits.length && hHits[0].properties.isHeightHandle) {
            // Start dragging height
            isDraggingVertex = true;
            draggedPolyId = parseInt(hHits[0].properties.polyId, 10);
...
map.on('mouseup', () => {
    ...
    if (isDraggingVertex) {
        if (draggedPolyId != null) {
            ...
            if (f && f.props.is3D && (f.kind === 'polygon' || f.kind === 'rectangle' || f.kind === 'circle')) {
                // height handle drag: update height
                const b = calcBounds(f);
                if (b) {
                    const centerLat = (b[0][1] + b[1][1]) / 2;
                    const heightMeters = (cursorLL[1] - centerLat) * 111320;
                    f.props.height = Math.max(0, Math.round(heightMeters));
                    syncDraw();
                    // Update editor display if open
                    if ($('popup-shape-editor').classList.contains('open')) {
                        $('eHeight').value = f.props.height;
                        $('eHeightDisplay').textContent = f.props.height;
                    }
                }
            }
        }
...
function openShapeEditor(id) {
    ...
    $('eMarkerControls').style.display = isMarker ? 'block' : 'none';
    if (isMarker) {
    ...
    $('eTextControls').style.display = isText ? 'block' : 'none';
    if (isText) {
    ...
    $('eImageControls').style.display = isImage ? 'block' : 'none';
    if (isImage) {
        $('eImageOpacity').value = f.props.opacity || 1;
    }
    ...
    if (f.kind === 'circle' && f.props.radiusMeters) {
        $('eCircleRadiusDisplay').style.display = 'block';
        const r = f.props.radiusMeters;
        $('eCircleRadiusVal').textContent = r > 1000 ? `${(r/1000).toFixed(2)} km` : `${Math.round(r)} m`;
    } else {
        $('eCircleRadiusDisplay').style.display = 'none';
    }
...
// Shape editor input handlers
function updateShapeProp(prop, value, isNumber = false) {
    const f = features.find(x => x.id === selectedId);
    if (f) {
        f.props[prop] = isNumber ? parseFloat(value) : value;
        syncDraw();
        markDirty();
        // Update displays
        if (prop === 'height') {
            $('eHeightDisplay').textContent = value;
        } else if (prop === 'baseHeight') {
            $('eBaseHeightDisplay').textContent = value;
        }
    }
}
...
$('eBorderOp').oninput = e => updateShapeProp('borderOpacity', e.target.value, true);
$('eWidth').oninput = e => updateShapeProp('width', e.target.value, true);
$('eFillOp').oninput = e => updateShapeProp('fillOpacity', e.target.value, true);
...
$('eMarkerSize').oninput = e => updateShapeProp('iconSize', e.target.value, true);
$('eMarkerSizeMode').onchange = e => updateShapeProp('sizeMode', e.target.value);
$('eTextVal').oninput = e => updateShapeProp('text', e.target.value);
$('eFontSize').oninput = e => updateShapeProp('fontSize', e.target.value, true);
$('eImageOpacity').oninput = e => {
    updateShapeProp('opacity', e.target.value, true);
    // Update map layer opacity
    const f = features.find(x => x.id === selectedId);
    if (f && f.kind === 'image') {
        const layerId = `image-layer-${f.id}`;
...
$('eHeight').oninput = e => {
    updateShapeProp('height', e.target.value, true);
    // Update 3D layer if exists
    const f = features.find(x => x.id === selectedId);
    if (f && f.props.is3D) {
        const layerId3d = `polygon-${f.id}-3d`;
        if (map.getLayer(layerId3d)) {
            map.setPaintProperty(layerId3d, 'fill-extrusion-height', parseFloat(e.target.value));
        }
    }
};
$('eBaseHeight').oninput = e => {
    updateShapeProp('baseHeight', e.target.value, true);
    const f = features.find(x => x.id === selectedId);
    if (f && f.props.is3D) {
        const layerId3d = `polygon-${f.id}-3d`;
        if (map.getLayer(layerId3d)) {
            map.setPaintProperty(layerId3d, 'fill-extrusion-base', parseFloat(e.target.value));
        }
    }
};
$('eIs3D').onchange = e => {
    const f = features.find(x => x.id === selectedId);
    if (f && (f.kind === 'polygon' || f.kind === 'rectangle' || f.kind === 'circle')) {
        ...
        if (f.props.is3D) {
            ...
            const layerId2d = `polygon-${f.id}-2d`;
            if (map.getLayer(layerId2d)) {
                map.setLayoutProperty(layerId2d, 'visibility', 'none');
            }
        } else {
            // Remove 3D layer
            const layerId3d = `polygon-${f.id}-3d`;
            if (map.getLayer(layerId3d)) map.removeLayer(layerId3d);
            const layerId2d = `polygon-${f.id}-2d`;
            if (map.getLayer(layerId2d)) {
                map.setLayoutProperty(layerId2d, 'visibility', 'visible');
            }
        }
        syncDraw();
        markDirty();
...
$('eResetImageSize').onclick = () => {
    const f = features.find(x => x.id === selectedId);
    if (f && f.kind === 'image') {
        // Reset to original coordinates? For simplicity, we just re-add source with same coords
        // Re-set source coordinates to current polygon
        const sourceId = `image-${f.id}`;
        if (map.getSource(sourceId)) {
            map.getSource(sourceId).setCoordinates(f.geometry.coordinates[0]);
        }
        hint('Image reset to original size.');
    }
};
...
$('btnDeleteSelected').onclick = () => {
    ...
    if (confirm(`Delete ${selectedLayerIds.size} selected layers?`)) {
...
function renderLayerCardHtml(f) {
    ...
    if (f.kind === 'circle' && f.props.radiusMeters) {
        ...
        subInfo = 'Image Overlay';
    } else if (f.props.is3D) {
        subInfo += ' (3D)';
    }
...
function openAttributeTable(featureId) {
    ...
    if (!f.props.attrTypes) {
        f.props.attrTypes = { description: 'text' };
    }
    if (!f.props.attrRows) {
        f.props.attrRows = [{ ...f.props.attributes }];
    }
    // Ensure 'name' is not included in attrTypes
    if (f.props.attrTypes.name) {
        delete f.props.attrTypes.name;
        f.props.attrRows.forEach(row => delete row.name);
    }
...
function renderAttributeTable(f) {
    ...
    // Exclude 'name' column
    const cols = Object.keys(types).filter(c => c !== 'name');
    const rows = f.props.attrRows || [{ ...f.props.attributes }];
    // Remove name from rows
    rows.forEach(row => delete row.name);
...
$('btnAddAttrCol').onclick = () => {
    ...
    if (name && name.trim()) {
        const c = name.trim();
        if (c === 'name') { hint('Column "name" is reserved.'); return; }
...
$('importFileInput').onchange = async function(e) {
    ...
    try {
        if (ext === 'geojson' || ext === 'json') {
            ...
            const latCol = Object.keys(rows[0]).find(k => /lat/i.test(k) || /latitude/i.test(k));
            const lonCol = Object.keys(rows[0]).find(k => /lon/i.test(k) || /longitude/i.test(k));
            if (!latCol || !lonCol) throw new Error('Could not find latitude/longitude columns.');
            rows.forEach(row => {
            ...
            hint(`Imported ${rows.length} points from CSV.`);
        } else if (['png','jpg','jpeg','webp'].includes(ext)) {
            // Image import handled in tool
            hint('Please use the Image Overlay tool to place images.');
        } else {
            hint(`Unsupported file type: ${ext}`);
        }
    } catch(err) {
...
function processGeoJSON(geojson) {
    ...
    feats.forEach(f => {
        ...
        if (geom.type === 'Point') {
            kind = 'marker';
            extraProps = { shape: 'pin', color: '#1e40af', iconSize: 0.9, iconKey: getIconKey('pin', '#1e40af'), attributes: props };
        } else if (geom.type === 'LineString' || geom.type === 'MultiLineString') {
            if (props.route_mode || props.routeMode) {
                kind = 'route';
                extraProps = { routeMode: props.route_mode || props.routeMode, description: props.description || '', color: props.color || '#38bdf8', borderColor: props.color || '#38bdf8', waypoints: geom.coordinates };
            } else {
                kind = 'polyline';
                extraProps = {};
            }
            ...
            kind = 'polygon';
            extraProps = { is3D: !!props.is3D, height: props.height || 0, baseHeight: props.baseHeight || 0 };
        }
        addFeatureRecord(kind, geom, extraProps);
    });
}
...
try:
    ...
    if ALL_PROJECTS_LIST:
        ...
        # Ensure features have 3D props
        for f in initial_features:
            if f.get("kind") in ["polygon", "rectangle", "circle"]:
                if "is3D" not in f.get("props", {}):
                    f["props"]["is3D"] = False
                if "height" not in f.get("props", {}):
                    f["props"]["height"] = 20
                if "baseHeight" not in f.get("props", {}):
                    f["props"]["baseHeight"] = 0
...
# ------------------------------------------------------------------------
# 6. PYTHON BACKEND: NOMINATIM GEOCODING
# ------------------------------------------------------------------------
_last_geocode_time = 0
def geocode_nominatim(query: str) -> tuple:
    """
    Forward geocode a place name using Nominatim API.
    Returns (lat, lon) or (None, None) on failure.
    Implements rate limiting (at least 1 second between calls).
    """
