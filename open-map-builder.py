...
# ------------------------------------------------------------------------
# 1. PAGE CONFIG — single screen, no scrolling
# ------------------------------------------------------------------------
...
st.markdown("""
    ...
    div[data-testid="stIFrame"] iframe { height: 100vh !important; width: 100%; border: none; }
...
# ------------------------------------------------------------------------
# 3. MAP + RAIL + DATA BROWSER (single-screen UI)
# ------------------------------------------------------------------------
...
<style>
  ...
  #panel { position: absolute; left: 60px; top: 8px; bottom: 8px; width: 300px; z-index: 3;
           ...
           padding: 12px; overflow-y: auto; display: none; flex-direction: column; gap: 7px;
  ...
  .sec { font-size: 10px; letter-spacing: 1px; color: #8b949e; margin-top: 4px; }
  ...
  #panel select { background: #0d1117; color: #f0f6fc; border: 1px solid #30363d;
...
<div id="rail">
  ...
  <button class="tool" data-tool="text" title="Add text"><svg viewBox="0 0 24 24" width="14" height="14" fill="currentColor"><path d="M4 4h16v4h-2V6h-5v14h-2V6H6v2H4z"></path></svg></button>
  ...
  <div class="tsep"></div>
  <button id="basemap-btn" title="Basemap / Custom"><svg viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="currentColor" stroke-width="2"><path d="M1 6v16l7-4 8 4 7-4V2l-7 4-8-4z"></path><line x1="8" y1="2" x2="8" y2="18"></line><line x1="16" y1="6" x2="16" y2="22"></line></svg></button>
...
<div id="panel" class="open">
  ...
  <div id="customSec" style="display:none">
    <div class="sec">CUSTOM MAP</div>
    <div class="row"><span>Background</span><input type="color" id="cBg" value="#0a1628"></div>
    <div class="row"><span>Main Roads</span><input type="color" id="cMainC" value="#e8b84a"></div>
    <div class="row"><span>Main width</span><input type="range" id="cMainW" min="0.5" max="3" step="0.1" value="1"></div>
    <div class="row"><span>Main opacity</span><input type="range" id="cMainO" min="0" max="1" step="0.05" value="1"></div>
    <div class="row"><span>Secondary</span><input type="color" id="cSecC" value="#7d5f14"></div>
    <div class="row"><span>Sec width</span><input type="range" id="cSecW" min="0.5" max="3" step="0.1" value="1"></div>
    <div class="row"><span>Sec opacity</span><input type="range" id="cSecO" min="0" max="1" step="0.05" value="0.7"></div>
    <div class="row"><span>Buildings</span><input type="color" id="cBldC" value="#8e7258"></div>
    <div class="row"><span>Bld opacity</span><input type="range" id="cBldO" min="0" max="1" step="0.05" value="0.07"></div>
    <div class="row"><span>Water</span><input type="color" id="cWatC" value="#0a1424"></div>
    <div class="row"><span>Labels</span><input type="color" id="cLblC" value="#d9b451"></div>
    <div class="row"><span>Label opacity</span><input type="range" id="cLblO" min="0" max="1" step="0.05" value="1"></div>
  </div>

  <div class="sec">MARKER ICON</div>
  <div id="iconRow"></div>

  <div class="sec">TEXT TOOL</div>
  <input id="textContent" type="text" placeholder="Text to place…"/>
  <div class="row"><span>Font</span>
    <select id="textFont">
      <option selected>Century Gothic</option><option>Arial</option>
      <option>Georgia</option><option>Courier New</option><option>Verdana</option>
    </select></div>
  <div class="row"><span>Size</span><input type="range" id="textSize" min="10" max="64" step="1" value="24"></div>
  <div class="row"><span>Opacity</span><input type="range" id="textOp" min="0.1" max="1" step="0.05" value="1"></div>
  <div class="row"><span>Color</span><input type="color" id="textColor" value="#d9b451"></div>
  ...
  <div id="editSec" style="display:none">
    <div class="sec">EDIT · <span id="editName"></span></div>
    <div class="row" id="rText" style="display:none"><span>Text</span><input type="text" id="eText" style="width:110px"></div>
    <div class="row" id="rColor"><span>Color</span><input type="color" id="eColor"></div>
    <div class="row" id="rWidth"><span>Width</span><input type="range" id="eWidth" min="1" max="12" step="1"></div>
    <div class="row" id="rSize" style="display:none"><span>Size</span><input type="range" id="eSize" min="10" max="64" step="1"></div>
    ...
    <span style="color:#8b949e;font-size:10px">drag shape/points · right-click point = delete</span></div>
  </div>
</div>
...
let features = [], texts = [], fid = 0, activeTool = null, pickMode = false;
let draft = [], routeA = null, cursorLL = null;
let editingId = null, editingTextId = null, drag = null;
let markerShape = 'pin';
...
const HINTS = {
  marker: 'Click to place markers', text: 'Click map to place text',
...
const textById = id => texts.find(x => x.id === id);
...
// ---------------- text tool (DOM markers: any CSS font works) ----------------
function styleTextEl(t) {
  t.el.textContent = t.text;
  t.el.style.fontFamily = `'${t.font}', 'Century Gothic', sans-serif`;
  t.el.style.fontSize = t.size + 'px';
  t.el.style.color = t.color;
  t.el.style.opacity = t.opacity;
  t.el.style.textShadow = '0 1px 4px rgba(0,0,0,0.55)';
  t.el.style.whiteSpace = 'nowrap';
  t.el.style.display = t.visible ? '' : 'none';
}
function addText(ll) {
  const t = { id: ++fid, kind: 'text', ll,
    text: $('textContent').value || 'Label', font: $('textFont').value,
    size: +$('textSize').value, opacity: +$('textOp').value,
    color: $('textColor').value, visible: true };
  t.el = document.createElement('div');
  styleTextEl(t);
  t.marker = new maplibregl.Marker({ element: t.el, anchor: 'center' })
    .setLngLat(ll).addTo(map);
  t.marker.on('dragend', () => { t.ll = t.marker.getLngLat().toArray(); });
  texts.push(t); refreshManager();
}
...
function openEditUI(name, isText) {
  $('panel').classList.add('open'); $('db-toggle').classList.add('active');
  $('editSec').style.display = 'block';
  $('editName').textContent = name;
  $('rText').style.display = isText ? 'flex' : 'none';
  $('rSize').style.display = isText ? 'flex' : 'none';
  $('rWidth').style.display = isText ? 'none' : 'flex';
}
function startEditing(id) {
  stopEditing();
  editingId = id;
  const f = byId(id); if (!f) return;
  openEditUI(f.kind + ' ' + id, false);
...
function startEditingText(id) {
  stopEditing();
  editingTextId = id;
  const t = textById(id); if (!t) return;
  t.marker.setDraggable(true);
  openEditUI('text ' + id, true);
  $('eText').value = t.text; $('eColor').value = t.color;
  $('eSize').value = t.size; $('eOp').value = t.opacity;
  hint('Drag the text to move it');
}
function stopEditing() {
  if (editingTextId != null) { const t = textById(editingTextId); if (t) t.marker.setDraggable(false); }
  editingId = null; editingTextId = null; drag = null;
  $('editSec').style.display = 'none';
  renderHandles();
  map.getCanvas().style.cursor = activeTool ? 'crosshair' : '';
}

// ---------------- custom map (live style editing) ----------------
const MAIN_L = ['rd_major', 'case_major_casing'];
const SEC_L = ['rd_min_hi', 'case_minhi_casing', 'rd_min_md', 'rd_min_lo', 'rd_path'];
let baseWidths = {};
function captureWidths() {
  baseWidths = {};
  [...MAIN_L, ...SEC_L].forEach(id => {
    if (map.getLayer(id)) baseWidths[id] = map.getPaintProperty(id, 'line-width');
  });
}
const setIf = (id, prop, val) => { if (map.getLayer(id)) map.setPaintProperty(id, prop, val); };
function applyCustom() {
  setIf('bg', 'background-color', $('cBg').value);
  const mw = +$('cMainW').value, mo = +$('cMainO').value;
  MAIN_L.forEach(id => { setIf(id, 'line-color', $('cMainC').value); setIf(id, 'line-opacity', mo);
    if (baseWidths[id]) setIf(id, 'line-width', ['*', baseWidths[id], mw]); });
  const sw = +$('cSecW').value, so = +$('cSecO').value;
  SEC_L.forEach(id => { setIf(id, 'line-color', $('cSecC').value); setIf(id, 'line-opacity', so);
    if (baseWidths[id]) setIf(id, 'line-width', ['*', baseWidths[id], sw]); });
  setIf('building', 'fill-color', $('cBldC').value); setIf('building', 'fill-opacity', +$('cBldO').value);
  setIf('building', 'fill-outline-color', $('cBldC').value);
  setIf('water', 'fill-color', $('cWatC').value); setIf('waterway', 'line-color', $('cWatC').value);
  setIf('label_place', 'text-color', $('cLblC').value); setIf('label_place', 'text-opacity', +$('cLblO').value);
}
['cBg','cMainC','cMainW','cMainO','cSecC','cSecW','cSecO','cBldC','cBldO','cWatC','cLblC','cLblO']
  .forEach(id => $(id).addEventListener('input', applyCustom));

// ---------------- measure readout (length + radius) ----------------
function showMeasure(e, txt) {
  const el = $('measure');
...
map.on('mousemove', e => {
  ...
  if (activeTool) {
    renderDraft();
    if (activeTool === 'circle' && draft.length === 1)
      showMeasure(e, 'R = ' + fmtDist(haversine(draft[0], cursorLL)));
    else if ((activeTool === 'polyline' || activeTool === 'polygon') && draft.length)
      showMeasure(e, 'L = ' + fmtDist(draftLength()));
    else hideMeasure();
  }
...
map.on('click', e => {
  ...
  else if (activeTool === 'text') addText(ll);
  ...
  if (editingId != null && !activeTool) {
    ...
    if (hf.length) {
      ...
      if (f.geometry.type === 'Polygon' && f.geometry.coordinates[0].length > 4) {
        ...
        r.splice(idx, 1); r[r.length - 1] = [...r[0]];
        syncDraw(); renderHandles();
...
// ---------------- editor bindings ----------------
$('eColor').oninput = e => {
  if (editingTextId != null) { const t = textById(editingTextId); t.color = e.target.value; styleTextEl(t); }
  else { const f = byId(editingId); if (f) { f.props.color = e.target.value;
    if (f.kind === 'marker') f.props.icon = ensureIcon(f.props.shape, f.props.color); syncDraw(); } }
};
$('eWidth').oninput = e => { const f = byId(editingId); if (f) { f.props.width = +e.target.value; syncDraw(); } };
$('eSize').oninput  = e => { const t = textById(editingTextId); if (t) { t.size = +e.target.value; styleTextEl(t); } };
$('eOp').oninput    = e => {
  if (editingTextId != null) { const t = textById(editingTextId); t.opacity = +e.target.value; styleTextEl(t); }
  else { const f = byId(editingId); if (f) { f.props.opacity = +e.target.value; syncDraw(); } }
};
$('eText').oninput  = e => { const t = textById(editingTextId); if (t) { t.text = e.target.value; styleTextEl(t); refreshManager(); } };
...
$('clearbtn').onclick = () => {
  features = []; texts.forEach(t => t.marker.remove()); texts = [];
  resetDraft(); stopEditing(); syncDraw(); refreshManager();
};
...
function refreshManager() {
  $('lyrCount').textContent = features.length + texts.length;
  const box = $('myLayers');
  const all = [
    ...features.map(f => ({ id: f.props.id, name: f.kind + ' ' + f.props.id, vis: f.props.visible, kind: f.kind })),
    ...texts.map(t => ({ id: t.id, name: 'text ' + t.id + ' · ' + t.text, vis: t.visible, kind: 'text' }))
  ];
  if (!all.length) { box.innerHTML = '<div style="color:#8b949e">No drawings yet. Use the draw tools to add shapes.</div>'; return; }
  box.innerHTML = all.slice().reverse().map(it =>
    `<div class="listitem">
       <button class="small" data-act="eye" data-id="${it.id}" title="Show/hide">${it.vis ? '👁' : '–'}</button>
       <span class="nm">${it.name}</span>
       <button class="small" data-act="edit" data-id="${it.id}" data-kind="${it.kind}" title="Edit">✎</button>
       <button class="small" data-act="zoom" data-id="${it.id}" data-kind="${it.kind}" title="Zoom to">⤢</button>
       <button class="small" data-act="del" data-id="${it.id}" data-kind="${it.kind}" title="Delete">✕</button>
     </div>`).join('');
  box.querySelectorAll('button[data-act]').forEach(b => b.onclick = () => {
    const id = parseInt(b.dataset.id, 10), kind = b.dataset.kind;
    if (b.dataset.act === 'eye') {
      if (kind === 'text') { const t = textById(id); t.visible = !t.visible; styleTextEl(t); }
      else { const f = byId(id); f.props.visible = f.props.visible ? 0 : 1; syncDraw(); }
      refreshManager();
    }
    if (b.dataset.act === 'edit') kind === 'text' ? startEditingText(id) : startEditing(id);
    if (b.dataset.act === 'del') {
      if (kind === 'text') { const t = textById(id); if (t) t.marker.remove(); texts = texts.filter(x => x.id !== id); }
      else { features = features.filter(x => x.props.id !== id); if (editingId === id) stopEditing(); syncDraw(); }
      refreshManager();
    }
    if (b.dataset.act === 'zoom') {
      let bnd = null;
      if (kind === 'text') { const t = textById(id); const [a, b2] = [t.ll, t.ll];
        bnd = [[a[0]-0.005, a[1]-0.005], [b2[0]+0.005, b2[1]+0.005]]; }
      else { const f = byId(id); bnd = boundsOf(f); }
...
// ---------------- basemap switcher + custom ----------------
$('basemap-btn').onclick = () => { $('panel').classList.add('open'); $('db-toggle').classList.add('active'); };
const BM_NAMES = [...Object.keys(ALL_STYLES), 'Custom Map'];
$('bmGrid').innerHTML = BM_NAMES.map(n =>
...
$('bmGrid').querySelectorAll('button').forEach(b => b.onclick = () => {
  ...
  const n = b.dataset.n;
  if (n === 'Custom Map') {
    // Custom starts from the Midnight vector style, then live-edits paints
    map.setStyle(ALL_STYLES['Midnight Blue']);
    map.once('idle', () => {
      rebuildAfterStyleSwap();
      captureWidths();
  ...
  map.setStyle(ALL_STYLES[n]);
  map.once('idle', rebuildAfterStyleSwap);
});
function rebuildAfterStyleSwap() {
...
try:
    ...
    components.html(html, height=700, scrolling=False)  # CSS forces 100vh
