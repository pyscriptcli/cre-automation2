...
def standalone_map_editor(token_key):
    ...
    with st.expander("🗺️ Configure Map", expanded=not map_state["has_image"]):
        ...
        # Build the map HTML - properly escaped
...
def build_map_html(lat, lon, zoom, style, color, size, token_key):
    """Build HTML for standalone map with Leaflet.js - properly escaped"""
    ...
    # Use string formatting instead of f-strings for HTML to avoid escaping issues
    html_template = """
    <!DOCTYPE html>
    <html>
    <head>
...
def open_map_in_new_tab(token_key):
    ...
    # Import button
    if st.button("📥 Import from Map", key=f"import_map_{token_key}", use_container_width=True):
        # Check if we have any data from the map
        if map_state.get("has_image") and map_state.get("image_bytes"):
            st.success("✅ Map imported successfully!")
            st.rerun()
        else:
            st.warning("No map data found. Please open the map editor, configure your map, and export it.")
