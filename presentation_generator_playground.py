...
def render_isolated_map_editor():
    ...
    # Apply map dragged state changes safely BEFORE rendering the widget
    ...
    with st.form(f"config_form_{token_key}"):
        st.markdown("##### Pin and View Settings")
        ...
        new_coord = c2.text_input("Coordinates (Lat, Lon)", key=coord_key)
        ...
        with c5:
            ...
            if st.form_submit_button("Update Map Preview", type="secondary"):
                ...
                st.session_state[style_key] = new_style
                st.session_state[color_key] = new_color
                st.session_state[size_key] = new_size
                st.session_state[coord_key] = f"{conf['lat']}, {conf['lon']}"
                st.rerun()
    ...
    st.info("Use the Rectangle tool (Square Icon) to frame your export area. Drag the pin to move it. The map will not flash while adjusting settings.")
