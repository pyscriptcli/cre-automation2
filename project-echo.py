...
# ---------- NEW: OpenAI Whisper fallback function ----------
def _call_openai_whisper(audio_bytes, filename="audio.mp3"):
    """Transcribe using OpenAI Whisper (fallback)."""
    if not OPENAI_API_KEY:
        return None
    headers = {"Authorization": f"Bearer {OPENAI_API_KEY}"}
    files = {"file": (filename, audio_bytes), "model": (None, "whisper-1")}
    try:
...
# ---------- Modified: Groq Whisper with rate‑limit handling ----------
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            if "rate limit" in error_msg.lower():
                ...
                if resp.status_code == 200:
                    return resp.json().get("text", "")
            st.error(f"Groq transcription error: {error_msg}")
            return None
    except Exception as e:
        st.error(f"Groq API connection error: {e}")
        return None

# ---------- NEW: Helper that tries Groq then falls back to OpenAI ----------
def transcribe_with_fallback(audio_bytes, filename, segment_idx=None):
    """Attempt Groq; if it fails, fall back to OpenAI."""
    # Try Groq first
    text = _call_groq_whisper(audio_bytes, filename)
    if text is not None:
        return text

    # Fallback to OpenAI if available
    if OPENAI_API_KEY:
        seg_info = f" (segment {segment_idx+1})" if segment_idx is not None else ""
        st.info(f"Groq rate limit reached{seg_info}, falling back to OpenAI Whisper.")
        return _call_openai_whisper(audio_bytes, filename)

    st.warning("OpenAI API key not provided. Cannot fallback.")
    return None
# ------------------------------------------------------------------------
    ...
    try:
        ...
        if comp_size_mb <= 22.0:
            ...
            with open(compressed_mp3, "rb") as f:
                c_bytes = f.read()
            # Use fallback helper
            text = transcribe_with_fallback(c_bytes, "compressed.mp3")
        ...
        for idx, seg in enumerate(segments):
            ...
            with open(seg, "rb") as f:
                seg_bytes = f.read()
            # Use fallback helper with segment index
            t = transcribe_with_fallback(seg_bytes, f"part_{idx}.mp3", segment_idx=idx)
            if t:
