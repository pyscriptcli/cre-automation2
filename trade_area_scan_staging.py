"""
Trade Area Scan - Complete Unified Version with Leaflet Geoman Control Integration
Features: SCAN module (POI discovery) + EDITOR module (drawing with Leaflet Geoman)
Unified client-side viewport controller with reactive state persistence.
"""
...
def init_session_state() -> None:
    """Initialize all session state variables with safe defaults and query overrides."""
    if "module" in st.query_params:
        st.session_state.active_module = st.query_params["module"]
...
# =====================================================================
# 7. SIDEBAR UI (NO REPLICATED SWITCH CONTROLS)
# =====================================================================
def render_sidebar() -> None:
    """Render context-aware sidebar controlled via unified client frame toggle."""
    with st.sidebar:
        st.markdown(
...
def render_sidebar_editor() -> None:
    ...
    st.markdown(
        "<div style='font-size:10px; font-weight:600; color:#888780; text-transform:uppercase; text-align:center; margin-bottom:12px;'>Layer Management</div>",
        unsafe_allow_html=True,
    )
...
# =====================================================================
# 8. MASTER LEAFLET TEMPLATE (UNIFIED VIEWPORT STATE ENGINE)
# =====================================================================
def get_leaflet_unified_template() -> str:
    """Return single combined HTML layout with client-side canvas control toggles."""
    return """
    <!DOCTYPE html>
    <html>
    <head>
        ...
        <style>
            ...
            /* Switch Control UI Placement */
            #mode-toggle-container {
                display: flex; background: #ffffff; padding: 2px;
                border-radius: 4px; border: 1px solid rgba(0, 51, 102, 0.1);
                box-shadow: 0 4px 12px rgba(0, 51, 102, 0.08); gap: 2px;
            }
            .mode-toggle-btn {
                padding: 6px 12px; font-size: 10px; font-weight: 700;
                text-transform: uppercase; letter-spacing: 0.5px; cursor: pointer;
                border-radius: 3px; color: #003366; transition: all 0.2s ease;
                font-family: 'Montserrat', sans-serif;
            }
            .mode-toggle-btn:hover { color: #C9AB4C; background: #f8fafc; }
            .mode-toggle-btn.active { background: #003366 !important; color: #ffffff !important; }

            #minimal-basemap-panel {
                position: absolute; top: 110px; left: 50px; z-index: 1000;
                background: #ffffff; border-radius: 4px; border: 1px solid rgba(0, 51, 102, 0.1); 
                display: none; flex-direction: column; padding: 4px; box-shadow: 0 4px 12px rgba(0, 51, 102, 0.08); 
                width: 150px;
            }
            #minimal-basemap-panel select {
                border: none; border-bottom: 1px solid #f0f0f0; padding: 6px; font-size: 10px; 
                font-weight: 700; font-family: 'Montserrat', sans-serif; color: #003366; 
                background: transparent; outline: none; cursor: pointer; width: 100%; text-transform: uppercase;
            }
            .minimal-label { 
                font-size: 9px; font-weight: 700; padding: 6px; display: flex; align-items: center; 
                gap: 4px; cursor: pointer; color: #888780; margin: 0; text-transform: uppercase; 
                border-top: 1px solid #f8fafc;
            }

            #search-container { position: absolute; top: 10px; left: 54px; z-index: 1000; width: 300px; }
            #map-search {
                width: 100%; padding: 8px 12px; border: 1px solid rgba(0, 51, 102, 0.1); 
                border-radius: 4px; background-clip: padding-box; font-size: 11px; 
                font-family: 'Montserrat', sans-serif; font-weight: 600; color: #003366; 
            ...
            #search-results { 
                ...
                border-radius: 2px; display: none; max-height: 250px; overflow-x: hidden; 
                overflow-y: auto; border: 1px solid rgba(0, 51, 102, 0.1); box-sizing: border-box; 
                z-index: 1001; box-shadow: 0 4px 12px rgba(0, 51, 102, 0.08);
            }
            .search-item { 
                padding: 8px 12px; font-size: 10px; font-weight: 600; cursor: pointer; 
                border-bottom: 1px solid #f8fafc; color: #003366; white-space: nowrap; 
            ...
            #scan-results-panel { 
                ...
                width: 250px; max-height: calc(100vh - 20px); border-radius: 2px; 
                border: 1px solid rgba(0, 51, 102, 0.1); background-clip: padding-box; 
                display: none; flex-direction: column; overflow: hidden; 
                box-shadow: 0 4px 12px rgba(0, 51, 102, 0.08);
            }
            .results-header { 
                background: #003366; color: #ffffff; padding: 10px 12px; font-size: 10px; 
                font-weight: 800; display: flex; justify-content: space-between; align-items: center; 
                text-transform: uppercase; border-bottom: 2px solid #C9AB4C; letter-spacing: 1px;
            }
            ...
            .layer-category-header { 
                background: #ffffff; padding: 8px 12px; display: flex; align-items: center; 
                justify-content: space-between; cursor: pointer; user-select: none; 
                transition: background 0.2s;
            }
            .layer-category-header:hover { background: #f8fafc; }
            .layer-header-left { display: flex; align-items: center; gap: 6px; font-size: 9px; 
            ...
            .results-item { 
                padding: 6px 12px 6px 28px; font-size: 9px; font-weight: 600; 
                color: #888780; display: flex; justify-content: space-between; 
            ...
            .poi-text-label { 
                background: #fff; border: 1px solid #003366; padding: 2px 4px; 
                border-radius: 2px; font-size: 9px; font-family: 'Montserrat', sans-serif; 
            ...
            .leaflet-control-custom-stack { 
                background: #fff; border: 2px solid rgba(0,0,0,0.2); border-radius: 4px; 
                overflow: hidden; display: flex; flex-direction: column;
            }
            .leaflet-control-custom-stack a { 
                display: flex !important; align-items: center; justify-content: center; 
                background: #fff; text-decoration: none; width: 34px; height: 34px; 
            ...
            /* Vector Properties & Layers Panels */
    ...
    <body>
        ...
        <script>
            const activeModule = "__ACTIVE_MODULE__";
            const map = L.map('map', { zoomControl: true, attributionControl: false }).setView([__LAT__, __LON__], 14);
            map.zoomControl.setPosition('topleft');

            // Floating Custom Mode Selector Integration
            const modeControl = L.control({position: 'topleft'});
            modeControl.onAdd = function (map) {
                const div = L.DomUtil.create('div', 'leaflet-bar leaflet-control');
                div.id = 'mode-toggle-container';
                div.innerHTML = `
                    <div class="mode-toggle-btn ${activeModule === 'SCAN' ? 'active' : ''}" onclick="switchMode('SCAN')">SCAN MODULE</div>
                    <div class="mode-toggle-btn ${activeModule === 'EDITOR' ? 'active' : ''}" onclick="switchMode('EDITOR')">VECTOR EDITOR</div>
                `;
                return div;
            };
            ...
            function switchMode(newMode) {
                if (newMode === activeModule) return;
                syncDrawingsToLocalStorage();
                localStorage.setItem('ts_scanned_records', JSON.stringify(pts));
                
                // Route parameters back into Python sidebar context natively
                const link = document.createElement('a');
                link.href = './?module=' + newMode;
                link.target = '_top';
                document.body.appendChild(link);
                link.click();
            }

            // Geoman Active Protocol Handling
            const isEditor = activeModule === 'EDITOR';
            map.pm.addControls({
                position: 'topleft',
                drawMarker: isEditor,
                drawPolygon: isEditor,
                drawPolyline: isEditor,
                drawCircle: isEditor,
                drawRectangle: isEditor,
                drawCircleMarker: isEditor,
                editMode: isEditor,
                dragMode: isEditor,
                cutPolygon: isEditor,
                removalMode: isEditor,
                rotateMode: isEditor
            });
            if (!isEditor) {
                setTimeout(() => {
            ...
            function generateShareLink(e) {
                e.preventDefault();
                const link = window.location.href.split('?')[0] + "?c=__LAT__,__LON__&r=__RADIUS__";
            ...
            function saveProjectSettings(e) {
                ...
                const projectData = {
                    ...
                    radius: __RADIUS__,
                    scanned_records: pts,
                    drawings: localStorage.getItem('ts_editor_drawings')
                };
            ...
            // =====================================================================
            // POI RECOVERY & PARSING DATA LOOPS
            // =====================================================================
            let pts = __GEOJSON__;
            const savedPts = localStorage.getItem('ts_scanned_records');
            if (savedPts && "__IS_STALE__" === "false") {
                pts = JSON.parse(savedPts);
            } else {
                localStorage.setItem('ts_scanned_records', JSON.stringify(pts));
            }
            ...
            // =====================================================================
            // VECTOR EDITOR ENGINE CORE OBJECT MANAGEMENT
            // =====================================================================
            ...
            function syncDrawingsToLocalStorage() {
                const serialized = allFeatures.map(f => {
                    const item = { type: f.type, featureId: f.featureId, layerId: f.layerId, data: f.data };
                    if (f.type === 'marker' || f.type === 'circle' || f.type === 'circlemarker') {
                        const ll = f.layer.getLatLng();
                        item.lat = ll.lat; item.lon = ll.lng;
                        if (f.type === 'circle') item.radius = f.layer.getRadius();
                    } else {
                        item.latlngs = f.layer.getLatLngs();
                    }
                    return item;
                });
                localStorage.setItem('ts_editor_drawings', JSON.stringify(serialized));
            }

            function loadSavedDrawings() {
                const drawingsJSON = localStorage.getItem('ts_editor_drawings');
                if (!drawingsJSON) return;
                const savedDrawings = JSON.parse(drawingsJSON);
                
                savedDrawings.forEach(d => {
                    let layerInstance;
                    if (d.type === 'marker') layerInstance = L.marker([d.lat, d.lon]);
                    else if (d.type === 'circle') layerInstance = L.circle([d.lat, d.lon], { radius: d.radius });
                    else if (d.type === 'polygon') layerInstance = L.polygon(d.latlngs);
                    else if (d.type === 'polyline') layerInstance = L.polyline(d.latlngs);
                    else if (d.type === 'circlemarker') layerInstance = L.circleMarker([d.lat, d.lon]);
                    
                    if (layerInstance) {
                        const featId = d.featureId || 'drawn_' + featureCounter++;
                        const feat = { layer: layerInstance, type: d.type, data: d.data, featureId: featId, layerId: d.layerId || activeLayerId };
                        layerInstance._featureId = featId;
                        layerInstance._layerId = feat.layerId;
                        
                        applyStyleToLayer(layerInstance, d.data, d.type);
                        if (d.type === 'circle') updateCircleTooltip(layerInstance);
                        
                        const cfg = getLayerConfig(feat.layerId);
                        if (cfg.visible !== false) layerInstance.addTo(map);
                        
                        allFeatures.push(feat);
                        attachContextMenu(layerInstance, feat);
                        
                        layerInstance.on('pm:edit', function() {
                            if (d.type === 'circle') updateCircleTooltip(layerInstance);
                            syncDrawingsToLocalStorage();
                        });
                    }
            ...
            function ctxEditFeature() {
                ...
                if (ctxTargetFeature && ctxTargetFeature.layer && ctxTargetFeature.layer.pm) ctxTargetFeature.layer.pm.enable();
            }
            ...
            function selectFeature(featureObj) {
                selectedFeature = featureObj; selectedLayer = featureObj.layer;
                const d = featureObj.data; const cfg = getLayerConfig(featureObj.layerId);
            ...
            function dismissPropertiesPanel() {
                ...
                selectedFeature = null; selectedLayer = null;
            }
            ...
            function deleteFeature(featureObj) {
                if (featureObj.layer) {
                    ...
                    if (featureObj.layer._radiusTooltip) map.removeLayer(featureObj.layer._radiusTooltip);
                }
            ...
            map.on('pm:create', function(e) {
                const shape = e.shape; const layer = e.layer; const cfg = getActiveLayerConfig(); const featId = 'drawn_' + featureCounter++;
                let data = {
                    name: shape.charAt(0).toUpperCase() + shape.slice(1) + ' ' + featureCounter,
                    color: cfg.color, fill_color: cfg.fill_color, fill_opacity: cfg.fill_opacity, weight: cfg.weight, icon_shape: cfg.icon_shape, icon_size: cfg.icon_size
                };
                ...
                if (type === 'circle') {
                    ...
                    if (layer.setIcon) layer.setIcon(renderVectorPinIcon(cfg.color, cfg.icon_shape, cfg.icon_size));
                }
                
                const feat = { layer: layer, type: type, data: data, featureId: featId, layerId: activeLayerId };
                layer._featureId = featId; layer._layerId = activeLayerId;
                
                allFeatures.push(feat);
                attachContextMenu(layer, feat);
                
                layer.on('pm:edit', function() { syncDrawingsToLocalStorage(); });
            ...
            function updateCircleTooltip(circleLayer) {
                ...
                if (circleLayer._radiusTooltip) {
                    ...
                    circleLayer._radiusTooltip = L.tooltip({ permanent: true, direction: 'center', className: 'radius-tooltip', offset: [0, 0] })
                    .setContent(label).setLatLng(circleLayer.getLatLng()).addTo(map);
                }
            }

            map.on('pm:remove', function(e) {
                const idx = allFeatures.findIndex(f => f.layer === e.layer);
                if (idx > -1) { allFeatures.splice(idx, 1); syncDrawingsToLocalStorage(); renderLayerPanel(); }
            });

            // Universal Window Configurations
            if (isEditor) {
            ...
            map.on('contextmenu', function(e) {
                const lat = e.latlng.lat; const lng = e.latlng.lng; const coordStr = lat.toFixed(5) + ", " + lng.toFixed(5);
                const menuHtml = `
            ...
            // Clear context menus on base maps clicks
...
# =====================================================================
# 9. UNIFIED RECONSTRUCTED MAP RENDER ENGINE
# =====================================================================
def render_unified_map() -> None:
    """Render unified map pane passing real-time arrays to Leaflet."""
    ...
    is_stale = (
        lat_coord != st.session_state.last_scan_lat
        or lon_coord != st.session_state.last_scan_lon
    )

    layer_config = json.dumps(st.session_state.editor_layers)
    active_layer = st.session_state.get("active_editor_layer", "")

    # Enrich structural array maps objects
    ...
    leaflet_html = (
        template.replace("__LAT__", str(render_lat))
        ...
        .replace("__ACTIVE_MODULE__", active_module)
    )
