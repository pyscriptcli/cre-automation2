...
# =====================================================================
# 7. SIDEBAR UI (NO DUPLICATES)
# =====================================================================
def render_sidebar() -> None:
    """Render sidebar with location input, layer selection, and action buttons."""
    with st.sidebar:
        st.markdown(
        ...
        with btn_col1:
            if st.button(
                ...
                type="secondary" if st.session_state.active_module == "SCAN" else "primary",
                key="btn_scan",
            ):
                st.session_state.active_module = "SCAN"
                st.rerun()
        with btn_col2:
            if st.button(
                "EDIT",
                use_container_width=True,
                type="secondary" if st.session_state.active_module == "EDITOR" else "primary",
                key="btn_editor",
            ):
...
# =====================================================================
# 8. LEAFLET MAP TEMPLATES (SCAN & EDITOR)
# =====================================================================
def get_leaflet_scan_template() -> str:
    """Return Leaflet map HTML for SCAN module (read-only POI viewing)."""
    return """
    <!DOCTYPE html>
    <html>
    <head>
...
def get_leaflet_editor_template() -> str:
    ...
    <head>
        ...
        <style>
            ...
            /* Professional styling for Geoman Toolbar */
    ...
    <body>
        ...
        <script>
            ...
            // 2. Global Geoman Editor Settings (Best Practice: Snapping & Consistency)
            map.pm.setGlobalOptions({
                snappable: true,
                snapDistance: 20, 
                allowSelfIntersection: false,
                hintlineStyle: { color: '#C9AB4C', dashArray: '5,5' },
                templineStyle: { color: '#003366', weight: 2 }
            });
            ...
            function syncGeometryState(featObj) {
                // Best Practice: Update the underlying data model whenever the UI changes.
                const l = featObj.layer;
                if (featObj.type === 'circle') {
                    featObj.data.radius = l.getRadius();
                    featObj.data.latlng = l.getLatLng();
                } else if (featObj.type === 'marker') {
                    featObj.data.latlng = l.getLatLng();
                } else {
                    featObj.data.latlngs = l.getLatLngs();
                }
            }

            function updateShapeTooltip(leafletLayer, shapeType) {
                // Best Practice: Contextual measurements for shapes
                let label = '';
                
                try {
                    if (shapeType === 'circle') {
                        const r = leafletLayer.getRadius();
                        label = r >= 1000 ? (r / 1000).toFixed(2) + ' km' : Math.round(r) + ' m';
                    } else if (shapeType === 'polygon' && typeof turf !== 'undefined') {
                        const geojson = leafletLayer.toGeoJSON();
                        const area = turf.area(geojson);
                        label = area >= 1000000 ? (area / 1000000).toFixed(2) + ' km²' : Math.round(area) + ' m²';
                    } else if (shapeType === 'polyline' && typeof turf !== 'undefined') {
                        const geojson = leafletLayer.toGeoJSON();
                        const length = turf.length(geojson, {units: 'meters'});
                        label = length >= 1000 ? (length / 1000).toFixed(2) + ' km' : Math.round(length) + ' m';
                    }
                } catch(e) { label = ''; }
                ...
                if (leafletLayer._shapeTooltip) {
                    leafletLayer._shapeTooltip.setContent(label);
                    leafletLayer._shapeTooltip.setLatLng(centerPoint);
                } else {
                    leafletLayer._shapeTooltip = L.tooltip({
                        permanent: true,
                        direction: 'center',
                        className: 'shape-tooltip',
                        offset: [0, 0]
                    }).setContent(label).setLatLng(centerPoint).addTo(map);
                }
            }
            ...
            function initializeFeaturesOnCanvas() {
                pts.forEach(p => {
                    ...
                    if (layerInstance) {
                        ...
                        attachLifecycleHooks(layerInstance, feat);
                    }
                });
                renderLayerPanel();
            }
            ...
            function attachLifecycleHooks(leafletLayer, featureObj) {
                ...
                // 3. Edit & Drag Handlers for Synchronization
                leafletLayer.on('pm:edit', function(e) {
                    syncGeometryState(featureObj);
                    updateShapeTooltip(leafletLayer, featureObj.type);
                });
                leafletLayer.on('pm:dragend', function(e) {
                    syncGeometryState(featureObj);
                    updateShapeTooltip(leafletLayer, featureObj.type);
                });
                leafletLayer.on('pm:rotateend', function(e) {
                    syncGeometryState(featureObj);
                });
            }
            ...
            function ctxEditFeature() {
                ...
                if (ctxTargetFeature && ctxTargetFeature.layer && ctxTargetFeature.layer.pm) {
                    // Activate global edit mode, but specifically target this layer
            ...
            function deleteFeature(featureObj) {
                if (featureObj.layer) {
                    ...
                    if (featureObj.layer._shapeTooltip) {
                        map.removeLayer(featureObj.layer._shapeTooltip);
                    }
                }
                    ...
                    allFeatures.forEach(f => {
                        if (f.layerId === layerId) {
                            if (cfg.visible) {
                                ...
                                if(f.layer._shapeTooltip) map.addLayer(f.layer._shapeTooltip);
                            } else {
                                map.removeLayer(f.layer);
                                if(f.layer._shapeTooltip) map.removeLayer(f.layer._shapeTooltip);
                            }
                        }
                    });
                    renderLayerPanel();
            ...
            map.on('pm:create', function(e) {
                ...
                // Initialize Styles
                if (type === 'circle' || type === 'polygon' || type === 'polyline') {
                    layer.setStyle({
                        color: cfg.color,
                ...
                // Sync underlying state immediately 
                syncGeometryState(feat);
                updateShapeTooltip(layer, type);
                
                // Attach strict edit handlers to the newly drawn layer
                attachLifecycleHooks(layer, feat);
                
                renderLayerPanel();
                selectFeature(feat); // Auto-select to prompt user to name it
            });
            
            // Clean up memory if removed via global Geoman trash tool
            map.on('pm:remove', function(e) {
                const layer = e.layer;
                const idx = allFeatures.findIndex(f => f.layer === layer);
                if (idx > -1) {
                    if (allFeatures[idx].layer._shapeTooltip) {
                        map.removeLayer(allFeatures[idx].layer._shapeTooltip);
                    }
...
def render_scan_map() -> None:
    ...
    template = get_leaflet_scan_template()
    leaflet_html = (
